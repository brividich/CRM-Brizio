...
## 2026-09-26 - Codex (integrazione centrale MFC/SNMP e reportistica in release/prod)

- Richiesta: integrare subito `feature/contatori-centrale-snmp` dopo avere verificato che la release distribuita `d7ab908f` non conteneva nessuno dei quattro commit della funzionalita'. Integrazione preparata sopra l'attuale `release/prod` (`4776678e`, quindi comprensiva anche dei fix successivi a `d7ab908f`) nel worktree isolato `temp/codex-release-contatori`, branch `integration/contatori-release-20260926`. Checkout condiviso non modificato; preservata la cartella utente non tracciata `parser pdf/`.
- Commit sorgente integrati: `c463cb85` (conflitto argparse `--version`), `d052f2e3` (centrale MFC/SNMP e ponte Asset), `716010f5` (scheduler e letture mensili), `ff230d6f` (reportistica Asset PDF/Excel/web). Conflitti solo documentali in `CHANGELOG.md`, `README.md` e `docs/AUTOMAZIONI.md`: conservate sia le novita' gia' presenti in release (incluso `security_cycle`) sia quelle Contatori/Asset; catalogo rigenerato dalla fonte e portato a 48 automazioni.
- File critici integrati: `django_app/contatori/urls.py` e `django_app/assets/urls.py` (nuove route locali protette), `django_app/automazioni/schedules.py` (tre schedule aggiunti), `django_app/assets/migrations/0120_reportistica_acl.py` (binding locali gestione/lettura, senza nuovi grant). Nessuna modifica a middleware, autenticazione, settings o routing globale. Dettagli tecnici e rischi restano nelle tre voci sorgente immediatamente sotto.
- Test/check: 118 test mirati verdi (`assets.tests_reporting`, `assets.tests_acl_impostazioni_gate`, `automazioni.test_pianificati_page`, `automazioni.test_scadenze_schedules`, `contatori`) sul risultato del merge; Django system check incluso senza errori. I traceback `QA failure` e `share sparita` sono eccezioni intenzionalmente simulate dai test. Verifiche finali aggiuntive: migration drift Assets/Contatori, help dei tre comandi, secret hygiene, conflitti residui e diff check.
- Backup creati: nessuno; Git e worktree isolato rendono il merge recuperabile. README aggiornato: si, preservando la descrizione Security corrente. CHANGELOG aggiornato: si (root/Django). AGENT_CHANGELOG aggiornato: si. Nessun DB DEV/TEST/PROD aziendale, segreto o apparato SNMP toccato; test su SQLite isolato.
- Esito: merge pronto per pubblicazione su `origin/release/prod`; nessun deploy server eseguito. Dopo il nuovo pacchetto: `migrate`, `setup_q_schedules`, riavvio web/qcluster esistente e verifica dei tre schedule. La produzione su `d7ab908f` resta invariata finche' non viene distribuita la nuova revisione.
- Rischi residui: collaudo SQL Server e apparati reali, grant ACL report ai ruoli reali, crescita archivio report senza retention automatica, eventuale task SNMP Windows preesistente da disattivare per evitare doppioni. Nessuna nuova dipendenza.

## 2026-09-26 - Codex (reportistica Asset programmata)

- Richiesta: sezione Reportistica in Impostazioni Asset, estrazioni automatiche a scadenza, storico PDF/Excel in stile portale e consultazione con andamento. Proseguito sul worktree `temp/codex-contatori-centrale-snmp`, branch `feature/contatori-centrale-snmp`; nessuna modifica al checkout condiviso o alla produzione. Controlli sessione/lock/file critici non presenti.
- File applicativi modificati: `django_app/assets/{models.py,reporting_forms.py,views_reporting.py,urls.py,tests_reporting.py,tests_acl_impostazioni_gate.py}`, `services/{reporting.py,reporting_exports.py}`, `migrations/{0119_assetreportschedule_assetreportrun.py,0120_reportistica_acl.py}`, `templates/assets/reporting_base.html`, `templates/assets/pages/{reporting_settings,reporting_archive,reporting_detail,gestione_admin,reports_dashboard}.html`; `django_app/automazioni/schedules.py`.
- Funzioni: piani una tantum/giornalieri/settimanali/mensili con filtri tipo/categoria/reparto e data, snapshot e file privati nel DB, download autenticati/auditati, grafico web/Excel e tabella PDF delle ultime 12 estrazioni omogenee. Include ultime letture MFC/SNMP collegate, senza contattare apparati. Snapshot completati immutabili; quelli gia' acquisiti sono conservati anche nei retry. Date visualizzate nel fuso del portale, Excel anti-formula, PDF con intestazioni condivise e tabelle impaginate. Nessuna dipendenza aggiunta.
- Affidabilita': vincolo univoco piano/scadenza, lock e compare-and-set accodamento, tre tentativi e recupero esecuzioni interrotte (anche dopo il terzo timeout), niente ricostruzioni retroattive se il worker e' fermo. Limite 10.000 asset; prefetch a blocchi e subquery per non superare i 2100 parametri SQL Server. Estrai ora non cambia la scadenza. Test ACL preesistente reso idempotente rispetto al permesso canonico ora seminato dalla migrazione.
- File critici per funzione: `django_app/assets/urls.py` (7 nuove route locali protette per impostazioni, archivio e download), `assets/migrations/0120_reportistica_acl.py` (binding a prefisso su `legacy.assets.admin_assets` e `legacy.assets.assets_reports`, senza nuovi grant), `automazioni/schedules.py` (sola aggiunta del dispatcher `assets_reportistica`, ogni minuto). Motivo: rendere la funzione raggiungibile, separare gestione da lettura e riusare lo scheduler richiesto. Nessun cambiamento a middleware, autenticazione, settings permanenti, routing o navigazione globali. Impatto: permessi esistenti applicati alle nuove superfici; ruolo Reports vede il perimetro completo dei report, non soltanto gli asset assegnati. Rischi: verificare grant reali in TEST e worker attivo al deploy.
- Documentazione: `README.md`, `CHANGELOG.md`, `django_app/CHANGELOG.md`, nuovo `docs/ASSET_REPORTISTICA.md`, rigenerato `docs/AUTOMAZIONI.md` (47 voci), `docs/ai/{03_BACKEND_MODULES,05_SECURITY_BOUNDARIES}.md`, questo registro e checkpoint. Tutte le nuove voci restano Unreleased per il bump coordinato di release.
- Test/check: 118 test mirati verdi (`assets.tests_reporting`, `assets.tests_acl_impostazioni_gate`, `automazioni.test_pianificati_page`, `automazioni.test_scadenze_schedules`, `contatori`); Django system check pulito, nessun drift Assets/Contatori, diff check pulito. Test includono permessi, GET negato alle azioni, form, deduplica manuale/automatica, mese 31/DST, timeout/retry, snapshot immutabili, filtri, MFC/SNMP, PDF valido e struttura/formule Excel. Migrazioni applicate solo al DB sintetico.
- QA visuale: browser di test dopo indisponibilita' del collegamento integrato previsto dalla skill browser; desktop 1365px, mobile 390px senza overflow, tema scuro. PDF sintetico renderizzato e verificato (2 pagine finali), file Excel verificato nei test. Il polling notifiche globale del test user sintetico risponde 403: estraneo alle nuove pagine, non modificato. Warning preesistente di query in AppConfig.ready durante shell, nessun errore nei check.
- Backup creati: nessuno. README aggiornato: si. CHANGELOG aggiornato: si (root/Django). AGENT_CHANGELOG aggiornato: si. Nessun database reale, segreto, media privato o apparato SNMP consultato; server/settings/script QA temporanei rimossi/arrestati a fine verifica, restano soltanto artefatti sintetici ignorati da Git.
- Esito: implementazione verificata localmente, pronta per revisione/integrazione nella release, NON distribuita. Rischi residui: collaudo SQL Server e worker reale, crescita DB senza retention automatica, timeout 110s per report estesi; dati SNMP riflettono l'ultima raccolta e non una lettura live. Nessun invio email o ricostruzione dei consumi mensili incluso.
- Controlli aggiuntivi finali: `secret_hygiene_check` verde; help reale `leggi_contatori_mensili` disponibile nel codice locale. Ricaricata la pagina dopo riavvio QA: data effettiva mostrata correttamente in CEST.
- Note per altro agente/Brizio: distribuire anche gli antenati del branch (`c463cb85`, `d052f2e3`, `716010f5`). L'errore PROD `Unknown command: leggi_contatori_mensili` indica codice/comando non disponibile nell'installazione selezionata: il solo migrate non lo installa. Dopo rilascio: `migrate`, `setup_q_schedules`, riavvio web/qcluster esistente; verificare `assets_reportistica` in Task pianificati, poi creare i piani dal portale. Non creare un secondo task Windows. Nessun merge/push/deploy eseguito.

## 2026-09-26 - Codex (scheduler esistente e raccolta MFC mensile)

- Richiesta: usare le automazioni periodiche gia' presenti nel portale per SNMP e automatizzare mensilmente le letture MFC. Verificato nel codice: worker django-q2/qcluster, catalogo `automazioni/schedules.py`, registrazione `setup_q_schedules`, pagina `/admin-portale/automazioni/pianificati/` con on/off durevole ed Esegui ora. Non e' stato verificato lo stato runtime del server PROD.
- Implementazione: due schedule aggiuntivi (`contatori_poll_snmp`, 5 minuti; `contatori_letture_mensili`, giorno 1 ore 08:00 nel fuso del portale). Nessun task Windows aggiuntivo. Dispatcher con job per apparato, timeout 110s e lock accodamento in cache condivisa con scadenza 1h. Errori dei singoli apparati visibili in django-q; le altre macchine continuano.
- Mensili: nuovo `LetturaMensileContatori` con vincolo macchina/mese e timestamp reale, migration 0007; prima lettura riuscita conservata, retry idempotenti, recupero solo delle mancanti, nessuna retrodatazione. Snapshot separati dalle letture trimestrali esistenti (il vecchio comando le sovrascriveva nello stesso trimestre). Ultimi 24 mesi nella scheda MFC e comando manuale `leggi_contatori_mensili`.
- File modificati: `django_app/automazioni/schedules.py`; `django_app/contatori/{models.py,services.py,tasks.py,admin.py,views.py,tests_schedules.py}`, `migrations/0007_letturamensilecontatori.py`, `management/commands/leggi_contatori_mensili.py`, `templates/contatori/macchina.html`; `README.md`, `CHANGELOG.md`, `django_app/CHANGELOG.md`, `docs/{AUTOMAZIONI,CONTATORI_AUTOMAZIONI}.md`, `docs/ai/03_BACKEND_MODULES.md`, questo registro e checkpoint. La rigenerazione automatica di AUTOMAZIONI.md riallinea anche la voce intake_verifiche_periodiche gia' esistente nel catalogo.
- File critico per funzione: `django_app/automazioni/schedules.py` (catalogo scheduler globale). Motivo: integrare i job richiesti nella gestione centralizzata esistente. Modifica precisa: sole due nuove specifiche; nessuna modifica alle cadenze/abilitazioni degli altri job. Impatto previsto: registrazione tramite il normale setup e visibilita' nella pagina esistente. Rischi: serve qcluster attivo con cache condivisa; eventuale task Windows SNMP separato va disattivato per evitare doppioni. Nessuna modifica a ACL, autenticazione, middleware, routing o settings permanenti.
- Test/check: 93 test mirati verdi (`contatori`, `automazioni.test_pianificati_page`, `automazioni.test_scadenze_schedules`), inclusi 10 nuovi su idempotenza, due mesi distinti, errori parziali, impossibilita' retrodatazione, dispatch/duplicati e registrazione cron. Migrazione applicata al DB sintetico QA, Django system check senza errori, migration drift assente e diff check verde. Fixture IP/documenti solo sintetici; nessuna interrogazione di rete reale.
- Backup creati: nessuno; worktree e branch dedicati gia' esistenti. README aggiornato: si. CHANGELOG aggiornato: si (root/Django). AGENT_CHANGELOG aggiornato: si. Settings temporanei di test rimossi. Esito: codice pronto per integrazione release, non installato ne' attivato in produzione.
- Rischi residui: collaudo SQL Server/apparati reali e worker PROD da eseguire al deploy; il mese identifica quello della rilevazione, non il mese di consumo; gli errori persistenti richiedono Esegui ora dopo il ripristino, entro il mese corrente. Stato del dispatcher significa accodamento, non esito dei job figli. Limite di un'ora per il lock residuo in caso di worker interrotto. Versione lasciata Unreleased per il bump coordinato di release.
- Note per altro agente/Brizio: stessa feature branch `feature/contatori-centrale-snmp`. Dopo deploy: `migrate contatori`, `setup_q_schedules`, riavvio del qcluster gia' esistente; poi verificare i due job in Task pianificati. Runbook completo `docs/CONTATORI_AUTOMAZIONI.md`. Nessuna configurazione o DB aziendale modificato.

## 2026-09-26 - Codex (centrale MFC/SNMP e ponte Asset)

- Richiesta: ampliare Contatori MFC con monitor SNMP generico e collegamento bidirezionale al registro Asset HUB.
- File applicativi: `django_app/contatori/{models.py,forms.py,services.py,snmp.py,views.py,urls.py,admin.py,tests_snmp_centrale.py}`, migration `0006_macchina_snmp_stato_macchina_snmp_tempo_risposta_ms_and_more.py`, command `management/commands/{collega_asset.py,poll_snmp_devices.py}`, `static/contatori/contatori.css`, template `templates/contatori/{_modulo_nav,dashboard,discovery,macchina,macchine,riconciliazione,snmp_centrale,snmp_dispositivo_detail,snmp_dispositivo_form,snmp_sonda_form}.html`; `django_app/assets/templates/assets/pages/asset_detail.html`.
- Documentazione: `README.md`, `CHANGELOG.md`, `django_app/CHANGELOG.md`, `docs/ai/{03_BACKEND_MODULES,05_SECURITY_BOUNDARIES}.md`, questo registro e `session_checkpoint.md`.
- Funzioni: salute persistita MFC, anagrafica dispositivi generici, sonde OID numeriche/testuali/timeticks con conversione e soglie, storico polling, dashboard e monitor responsive, polling manuale e comando schedulabile. Ponte con FK Asset manuale o match univoco seriale/IP; automatch web solo alla creazione, rimozione manuale rispettata; `collega_asset` dry-run predefinito. Scheda Asset con link di ritorno e stato delle MFC/dispositivi associati.
- Robustezza: GET-only, OID validati, errori per singola sonda, numeri non finiti/fuori range respinti, errore form per OID duplicati, porta validata; dettaglio carica un solo ultimo valore per sonda; CSS limitato al contenitore del modulo.
- File critico per funzione: `django_app/contatori/urls.py`. Motivo: pubblicare monitor, schede, form e azioni POST di polling sotto il prefisso locale esistente `/contatori/`. Impatto: nuove route del modulo, protette dal middleware ACL esistente; nessuna modifica ad ACL, middleware, autenticazione, settings o routing/navigazione globali. Rischio residuo: verificare accesso dei ruoli reali nel collaudo TEST, come per le altre route del modulo. Test: rendering form/monitor/schede, integrazione Asset, polling POST-only.
- Backup: nessuno; isolamento nel worktree `temp/codex-contatori-centrale-snmp`, branch `feature/contatori-centrale-snmp`, derivato dal fix parser `c463cb85`. README aggiornato: si. CHANGELOG aggiornato: si (root e Django). AGENT_CHANGELOG aggiornato: si.
- Verifiche: 71/71 test `contatori` verdi (18 nuovi); Django system check senza errori; `makemigrations contatori --check --dry-run` senza drift; migrazione applicata su SQLite QA. Verifica visuale desktop e mobile con dati sintetici, corretto overflow a 390px. Nessuna interrogazione SNMP verso apparati reali. File/settings/server temporanei QA rimossi o arrestati; rimane solo DB di test ignorato da Git.
- Esito: implementazione completata e pronta per revisione/inclusione nella release. Non distribuita in produzione. Nessuna nuova dipendenza, nessuna modifica a segreti o database aziendali. Versione mantenuta nella sezione Unreleased: bump coordinato con il prossimo rilascio, senza cambiare i file runtime di deploy.
- Rischi residui: supporto SNMP v1/v2c, OID proprietari da configurare per ogni modello; durata indicata relativa all'intera interrogazione; storico senza retention automatica; polling web sincrono da preferire a piccoli gruppi, comando per parchi estesi. Verifica SQL Server e apparati reali da eseguire nell'ambiente TEST.
- Note per altro agente/Brizio: integrare anche il commit antenato del fix `--snmp-version`; applicare `migrate contatori` (0006), aggiornare gli statici, facoltativamente `collega_asset` e poi `collega_asset --apply`. `poll_snmp_devices` non viene schedulato automaticamente. Conservare modifiche estranee nel checkout condiviso (`parser pdf/`, `gestione_list.html`).
