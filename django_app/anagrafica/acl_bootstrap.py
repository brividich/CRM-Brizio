...
_BOOTSTRAP_CACHE_KEY = "anagrafica_acl_bootstrap_v1"
    ...
    if pulsante is None:
        try:
            pulsante = Pulsante.objects.create(
                codice=codice, nome_visibile=label, modulo=modulo, url=url, icona="users"
            )
            changed = True
        except DatabaseError:
            return None, False
        ...
        with transaction.atomic():
            for defn in _PULSANTI_DEFINITIONS:
                ...
                if pid and defn.get("hide"):
                    _hide_pulsante(pid, "anagrafica_api")
                changed = changed or item_changed
    except Exception as exc:
        logger.debug("ACL bootstrap anagrafica skipped: %s", exc)
        return
