...
_DISPOSITIVE_ACTIONS = {"prendi_in_carico", "approva", "respingi", "richiedi_modifica", "chiudi"}
...
def mail_action_view(request: HttpRequest, token: str) -> HttpResponse:
    ...
    first_images = []
    if token_obj.anomalie_ids:
        first_images = _load_first_images(token_obj.anomalie_ids[0])
...
def _handle_post(request: HttpRequest, token_obj, anomalie_live: list[dict]) -> HttpResponse:
    ...
    results = []
    if is_dispositive:
        target_avanzamento = nuovo_avanzamento or _ACTION_TO_AVANZAMENTO.get(action, "")
        for anomalia in anomalie_live:
            ...
            ok = _apply_action_to_anomalia(
                ...
                op_id=token_obj.op_id,
                action=action,
                nuovo_avanzamento=target_avanzamento,
                note=note,
            )
            results.append({
                "id": anomalia_id,
                "ok": ok,
                "prev": prev,
                "new": target_avanzamento if ok else prev,
            })
