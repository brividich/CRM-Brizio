"""Le impostazioni del Diario Preposto devono essere concedibili via ACL v2.

`_can_manage_settings`, la view `impostazioni` e l'API `api_cerca_utenti`
decidevano con `is_legacy_admin(...)`, vero solo per il ruolo "admin": nessun
altro ruolo poteva essere abilitato dal modulo permessi. In più `impostazioni` e
`api_cerca_utenti` non avevano nemmeno il bypass superuser, quindi negavano la
pagina a un superuser Django senza utenza legacy admin.
"""
...
from .acl_bootstrap import PERM_IMPOSTAZIONI_MANAGE, bootstrap_diario_preposto_acl_endpoints
...
class DiarioPrepostoImpostazioniAclTest(TestCase):
    def setUp(self):
        ...
        bootstrap_diario_preposto_acl_endpoints(force=True)
        self.user = self._make_user("dp-direzione", "direzione", RUOLO_DIREZIONE_ID)
    ...
    def _request(self, user=None):
        request = self.factory.get("/diario-preposto/impostazioni/")
    ...
    def test_bootstrap_registra_il_permesso(self):
        self.assertTrue(
            PermissionDefinition.objects.filter(
                code=PERM_IMPOSTAZIONI_MANAGE, is_active=True,
            ).exists()
        )
    ...
    def test_admin_legacy_resta_ammesso(self):
        admin_user = self._make_user("dp-admin", "admin", RUOLO_ADMIN_ID)
        cache.clear()
        bump_legacy_cache_version()
    ...
    def test_superuser_resta_ammesso(self):
        su = User.objects.create_superuser("dp-su", "dp-su@example.local", "pass12345")

        self.assertTrue(_can_manage_settings(self._request(su)))


@override_settings(LEGACY_AUTH_ENABLED=False, SECURE_SSL_REDIRECT=False)
class DiarioPrepostoViewGateTest(TestCase):
    """La view e l'API devono usare lo stesso cancello, superuser incluso."""
    ...
    def setUp(self):
        ...
        cache.clear()
        bootstrap_diario_preposto_acl_endpoints(force=True)
        self.su = User.objects.create_superuser("dp-su2", "dp-su2@example.local", "pass12345")
        bump_legacy_cache_version()

    def test_superuser_apre_le_impostazioni(self):
        from django.urls import reverse

        self.client.force_login(self.su)

        resp = self.client.get(reverse("diario_preposto:impostazioni"))

        self.assertEqual(resp.status_code, 200)

    def test_superuser_puo_cercare_utenti(self):
        from django.urls import reverse

        self.client.force_login(self.su)

        resp = self.client.get(reverse("diario_preposto:api_cerca_utenti"), {"q": "ab"})
