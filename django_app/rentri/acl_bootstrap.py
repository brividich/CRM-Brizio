...
_BOOTSTRAP_CACHE_KEY = "rentri_acl_bootstrap_v1"
    ...
    if pulsante is None:
        try:
            pulsante = Pulsante.objects.create(
                codice=codice, nome_visibile=label, modulo=modulo, url=url, icona="trash-2"
            )
            changed = True
        except DatabaseError:
            return None, False
        ...
        with transaction.atomic():
            for defn in _PULSANTI_DEFINITIONS:
                ...
                if pid and defn.get("hide"):
                    _hide_pulsante(pid, "rentri_api")
                changed = changed or item_changed
    except Exception as exc:
        logger.debug("ACL bootstrap rentri skipped: %s", exc)
        return
