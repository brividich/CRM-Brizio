...
- **[feat/test] `gestione_specifiche/templates/registro_ofi/lista.html`, `gestione_specifiche/tests/test_registro_ofi.py`**: le colonne P/D/C/A del MOD.174 sono **in fondo** alla tabella (dopo ~17 colonne), quindi invisibili senza scorrere in orizzontale; l'utente le vuole a colpo d'occhio. Aggiunta una colonna **«STATO PDCA»** compatta **subito dopo «OFI/NC»** (in testa, sempre visibile) con un **indicatore a 4 segmenti P·D·C·A** che mostra **tutte e quattro le tappe** insieme: raggiunte in **verde** (`ofi-mini-done`/`ofi-mini-current`, con la tappa corrente cerchiata d'arancio), da fare in **grigio** (`ofi-mini-todo`) — riusa la property `pdca_steps`. Le colonne MOD.174 in fondo (P·D·C·A·TOT, X cumulative) **restano invariate** per fedeltà al foglio. 1 nuovo test (`test_lista_indicatore_pdca_compatto_in_testa`: 4 segmenti, 2 done + 1 current + 1 todo per una voce in CHECK); suite `test_registro_ofi` 35 verdi. Nessuna migrazione.
