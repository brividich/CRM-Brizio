...
- **[perf] `tasks/action_register.py` (`annotate_open_action_counts`)**: nuova funzione che annota `open_action_count`/`overdue_action_count` su un queryset di progetti con sei sottoquery correlate (una per sorgente × metrica), non `Count()` incatenati sullo stesso queryset — annotare più `Count()` su relazioni diverse (issue/task/subtask) nella stessa query ne gonfia il risultato per fan-out dei JOIN. Usata da `project_list` per evitare N+1 sulla card.
- **[fix] `tasks/views.py` (`project_list`)**: `earliest_due` passa da `F("tasks__due_date")` a una sottoquery `Min()` correlata. Il campo F() su una relazione multi-riga forzava Django a raggruppare anche gli altri `Count()` annotati per quel valore, sdoppiando ogni progetto con più task in una riga per ogni `due_date` distinta — bug preesistente, invisibile finché nessuna vista mostrava un valore diverso per riga duplicata; la nuova pill "giorni alla consegna" lo rendeva visibile (stesso progetto due volte con scadenze diverse). Corretto perché necessario alla correttezza della nuova pill.
- **[fix] `tasks/views.py`**: rimosso un `from django.db.models import Min` locale nel ramo `view=timeline` che, dopo l'import di `Min` a livello modulo per il punto precedente, avrebbe reso `Min` una variabile locale non definita per tutta la funzione (`UnboundLocalError`) — rilevato da `TaskViewsImportHygieneTests`.
- Nuovi test: `tests_action_register.AnnotateOpenActionCountsTests` (parità con `count_project_open_actions`, zero azioni, `assertNumQueries(1)` su 12 commesse); `tests.TaskProjectsAndAttachmentsTests.test_project_list_query_budget_with_many_projects` (budget fisso a 8 query su 12 commesse, invariato al crescere del portfolio). `tests_project_overview.py` aggiornato per la classe rinominata `pf-name-link` → `pf-card-name`. Verifica manuale in tema chiaro/scuro (screenshot Playwright). `python manage.py test tasks` (231 test), `check`, `makemigrations --check --dry-run` e `secret_hygiene_check` verdi.
...
### Assets — manutenzione: priorità, scadenza, stato operativo e home "Il mio turno" per il manutentore

- **[feat/test] `assets/models.py` (`WorkOrder.priority`, `WorkOrder.due_at`, `WorkOrder.started_at`, `WorkOrder.operational_state`, `WorkOrder.start()`, `WorkOrder.is_overdue`), `assets/migrations/0095_workorder_due_at_workorder_priority_and_more.py` [nuovo]**: l'OdL dichiara ora una priorità (Urgente/Normale/Bassa, default Normale) e una scadenza esplicita opzionale (`due_at`), invece di dedurre l'urgenza dall'anzianità di apertura. Lo stato operativo (da assegnare/assegnato/in corso/in attesa) è una **property derivata** da `assigned_to`/`started_at`/`is_waiting` — non un campo separato da tenere in sync, per non introdurre un secondo stato che diverge dagli altri tre. `start()` è idempotente e riprende automaticamente da un'eventuale attesa (un solo bottone "Inizia/Riprendi" per il manutentore). Il "in attesa" con motivo esisteva già (`is_waiting`/`wait_reason`); non è stato duplicato.
- **[feat/test] `assets/views.py` (`workorder_detail` — azioni `start` e `set_priority`), `assets/templates/assets/pages/workorder_detail.html`**: bottone "Inizia intervento"/"Riprendi lavorazione" in toolbar; badge priorità/scadenza/stato operativo nell'header; azione rapida "Cambia priorità" (per il responsabile in fase di triage); campi "Iniziato il"/"Priorità"/"Scadenza" nel pannello panoramica. La schermata di dettaglio resta un'unica pagina (asset, rischi impliciti nel piano, checklist, allegati, azioni) invece di introdurne una nuova — coerente con "consolidare, non aggiungere pagine".
- **[feat/test] `assets/forms.py` (`WorkOrderForm`), `assets/templates/assets/pages/workorder_form.html`**: priorità e scadenza selezionabili alla creazione dell'OdL (entrambe opzionali).
- **[feat/test] `assets/views.py` (`_workorder_priority_order_case`, filtro `priority`), `assets/templates/assets/pages/workorder_list.html`**: nuovo filtro priorità con chip; nelle viste operative (aperti/miei/non assegnati) l'ordinamento mette prima gli urgenti e poi il più vecchio aperto (coda di lavoro), non l'ultimo creato; badge priorità/scaduto e colonna scadenza in tabella.
- **[feat/test] `assets/views.py` (`il_mio_turno`), `assets/urls.py` (`/assets/manutenzione/il-mio-turno/`), `assets/templates/assets/pages/il_mio_turno.html` [nuovo]**: home dedicata al manutentore, distinta dal Centro Manutenzione del responsabile. Sezioni **Bloccati → Emergenze → Scaduti → Oggi → In corso → Altri assegnati a te → Da prendere in carico**, ciascun OdL compare in una sola sezione (la prima che gli si applica). "Da prendere in carico" usa il claim esistente (`wo_claim`) con redirect di ritorno alla pagina, non un link ingannevole. Voce di navigazione aggiunta al fallback di default della sidebar Assets — **in prod, dove la sidebar è configurata da `AssetSidebarButton` nel pannello admin, va aggiunta manualmente** (la sidebar non è hardcoded, vedi `docs/ai/10_MAINTENANCE_MODERNIZATION.md`).
- **[docs] `docs/ai/10_MAINTENANCE_MODERNIZATION.md`**: Fase 5 documentata come completata; Fase 6 (checklist tipizzate, ricambi strutturati, autosalvataggio, offline) resta roadmap.
- Verificato via richieste HTTP autenticate dirette (login, `/assets/manutenzione/il-mio-turno/`, dettaglio OdL) su server di sviluppo: tutte le sezioni si popolano correttamente e ogni OdL seminato compare nella sezione attesa. **Non è stato un controllo visivo reale**: quel server dev risultava poi puntato al SQL Server locale reale (`config.settings.dev` non è SQLite su questa macchina), non un DB usa-e-getta — verificare aspetto e tema chiaro/scuro nel browser prima del deploy.

### Assets — manutenzione: checklist tipizzate, blocco chiusura, causa guasto, autosalvataggio

- **[feat/test] `assets/models.py` (`WorkOrderChecklist.step_type/is_mandatory/unit/range_min/range_max/value_numeric/value_text/is_skipped/skip_reason`, `WorkOrderAttachment.checklist_item`, `MaintenanceChecklistStep` stessi campi), migration `assets/0095_maintenancecheckliststep_is_mandatory_and_more.py` [nuova]**: la checklist OdL supporta ora 4 tipi di step oltre al sì/no esistente — **Misura** (valore numerico + range min/max opzionale, badge "fuori range"), **Testo**, **Foto** (allegato collegato allo step). Uno step marcato **obbligatorio** blocca la chiusura dell'OdL finché non è completo o **saltato con motivazione esplicita** (principio del report: "impedire la chiusura, oppure richiedere motivazione"). Gli step tipizzati si definiscono sul template intervento (`MaintenanceChecklistStep`, autorabili da `gestione_admin` e da Django admin) e vengono copiati nell'OdL alla creazione (`copy_template_checklist_to_workorder`).
- **[feat/test] `assets/views.py` (`workorder_checklist_set_value`, `workorder_checklist_photo`, `workorder_checklist_skip`, blocco in `workorder_close`), `assets/templates/assets/components/workorder_checklist.html`, `assets/templates/assets/pages/workorder_detail.html`**: nuove azioni HTMX per valorizzare/saltare uno step; `workorder_close` rifiuta la chiusura (con elenco degli step mancanti) se esistono step obbligatori incompleti e non saltati.
- **[feat/test] `assets/models.py` (`WorkOrder.failure_cause`), `assets/forms.py` (`WorkOrderCloseForm`), `assets/templates/assets/pages/workorder_close.html`**: causa del guasto codificata (Elettrica/Meccanica/Idraulica/Usura/Errore operatore/Altro, opzionale) invece di solo testo libero in `resolution` — serve per capire se un guasto si ripete per lo stesso motivo.
- **[feat] `assets/templates/assets/pages/workorder_close.html`**: autosalvataggio bozza del form di chiusura in `localStorage` (per-viewer, cancellato all'invio riuscito) — se il tablet si blocca o la pagina si ricarica per sbaglio il manutentore non riparte da zero. Nessun draft server-side.
- **[docs] `docs/ai/10_MAINTENANCE_MODERNIZATION.md`**: Fase 6 (parziale: punti 1/3/4 del backlog — checklist tipizzate, causa guasto, autosalvataggio) documentata come completata; ricambi strutturati e offline-first restano roadmap.
- 11 test nuovi, 422/424 test `assets` verdi (2 falliti preesistenti non collegati). **Nessun controllo visivo nel browser**: il tool di automazione non era disponibile e il tentativo di seed su server dev è stato interrotto — vedi nota sotto.
- **⚠️ Incidente di sessione**: durante la verifica manuale è emerso che `django_app/.env` del checkout condiviso punta a un **SQL Server locale reale** (`localhost\SQLEXPRESS`, DB `PORTALE NOVICROM`) anche per `config.settings.dev`, non a SQLite. Questa sessione e la precedente (Fase 5) hanno entrambe eseguito `manage.py migrate --settings=config.settings.dev` copiando quel `.env` in worktree separati, applicando su quel DB **due migration `assets/0095` diverse e non mergiate** (Fase 5: `workorder_due_at_workorder_priority_and_more`; Fase 6: `maintenancecheckliststep_is_mandatory_and_more`) — entrambe ora registrate come applicate. Righe di test create durante la verifica (`visual.check.fase5`/`fase6`, asset `VC-FASE5-01`/`VC-FASE6-01`) sono state ripulite. Le colonne restano sul DB locale: innocue finché i nomi migration non cambiano in fase di merge (vedi `docs/ai/10_MAINTENANCE_MODERNIZATION.md` e memoria `db_locale_dev_e_sqlserver_non_sqlite`) — **da verificare con `makemigrations --check` dopo ogni merge**, come da regola di progetto.
...
- **[feat/test] `anagrafica/models_formazione.py` (`TrainingScanIntakeConfig`, `TrainingScanLog.presenze_scritte`), `anagrafica/migrations/0104_trainingscanintakeconfig_and_more.py` [nuovo], `anagrafica/services/intake_scansioni.py` [nuovo], `anagrafica/tasks.py` (`run_intake_scansioni_formazione`), `automazioni/schedules.py` (`intake_scansioni_formazione`), `anagrafica/management/commands/intake_scansioni_formazione.py` [nuovo]**: **acquisizione dei fogli firme da cartella di rete**. Il gesto eliminato è banale e per questo pesante — aprire il portale, cercare la giornata, caricare il file: ora si preme «Scansione» sulla fotocopiatrice e basta. Funziona perché il foglio **si autodescrive**: il QR contiene il token, il token dice quale foglio, il foglio dice quale giornata e con quale elenco di persone, quindi **nessuna convenzione sul nome del file** — che è la prima cosa che nessuno rispetta. A ogni passaggio (schedule django-q2 ogni 2 minuti, `"I"`/MINUTES) per ciascun file: si legge il QR, si risolve il foglio, si **archivia** nell'archivio privato, si misura l'inchiostro cella per cella, si scrive la riga nel registro letture con `origine=CARTELLA`; poi il file si sposta in `elaborati\` o `errori\`, così la cartella d'ingresso resta pulita e **niente viene letto due volte**. Prudenze deliberate: un file ancora in scrittura (la fotocopiatrice scrive a pezzi) viene lasciato e ripreso al giro dopo, un limite per passaggio impedisce a un arretrato di bloccare il lavoro periodico, e un guasto sul singolo file non ferma gli altri. **Opt-in e fail-safe**: spento finché non lo si accende, e una share irraggiungibile è un contrattempo annotato, non un errore del portale. Comando `manage.py intake_scansioni_formazione [--limite N] [--forza]` per provare la configurazione dal server senza aspettare il giro.
- **[feat/test] `anagrafica/services/formazione_presenze.py` [nuovo], `anagrafica/views.py` (`formazione_registro_autocompila`)**: **una sola strada di scrittura** delle presenze. Le firme di una giornata possono ora arrivare da due parti — la pagina di conferma e la cartella di acquisizione — e se ciascuna si scrivesse le presenze per conto proprio prima o poi divergerebbero (una ricalcola le ore, l'altra no; una marca il metodo di firma, l'altra se lo dimentica): sarebbero due verità sullo stesso fatto, e in audit una delle due sarebbe sbagliata. La logica è estratta in `applica_firme_lezione(lezione, {id: (ingresso, uscita)})` e la view esistente ora la chiama, **stesso comportamento** (`PRESENTE` con entrambe le firme, `PARZIALE` con una sola, chi non ha firme non viene toccato, ricalcolo ore per iscritto). Il metodo di firma resta `UPLOAD` anche da cartella: da dove sia entrata nel portale lo dice già `origine` sul registro.
- **[feat/test] `anagrafica/forms.py` (`TrainingScanIntakeConfigForm`), `anagrafica/views.py` (`formazione_scansioni_impostazioni`), `anagrafica/urls.py`, `anagrafica/templates/anagrafica/pages/formazione_scansioni_impostazioni.html` [nuovo], `anagrafica/templates/anagrafica/pages/formazione_scansioni_log.html`**: pagina **Acquisizione scansioni** (`/anagrafica/formazione/scansioni/impostazioni/`, gated come le altre modifiche alla formazione): cartella, interruttore generale, spostamento dei file, file per passaggio, più il pulsante **«Prova adesso»** che esegue un passaggio immediato — vale più di ogni testo d'aiuto, perché una share o si raggiunge o non si raggiunge e chi configura deve saperlo subito invece di dedurlo da una cartella che resta vuota. In testa lo **stato reale** (cartella raggiungibile, acquisizione attiva/spenta, esito dell'ultimo passaggio). Un percorso con **lettera di unità** non viene rifiutato — può essere un disco locale del server — ma segnalato: se è un'unità mappata il servizio non la vede e la cartella resterebbe eternamente vuota senza che nessuno capisca perché.
- **[feat] `TrainingScanIntakeConfig.conferma_automatica` + `auto_solo_senza_dubbie` + `auto_solo_se_tutti_firmati`**: la **conferma resta umana per default**, e non per prudenza formale — la lettura misura se una cella contiene inchiostro, **non riconosce chi ha firmato**, e la presenza a un corso è un atto con valore legale. L'automatismo si accende consapevolmente dalla pagina, con l'avvertenza esplicita accanto all'interruttore, e anche acceso si ferma davanti alle **celle dubbie** (dove il segno è troppo debole per decidere) e, se richiesto, davanti ai fogli in cui **non tutti gli attesi risultano firmati** — utile per accorgersi dei fogli letti a metà. Quando si ferma, il motivo finisce nel registro accanto alla lettura. Il conteggio delle presenze scritte da sé è sulla riga di registro (`presenze_scritte`), quindi in audit si distingue sempre ciò che ha registrato una persona da ciò che ha registrato il portale.
- **[test] `anagrafica/tests_formazione_intake_scansioni.py` [nuovo]**: **20 test** sul giro vero — si emette il foglio, lo si rasterizza, ci si «firma» sopra, lo si scrive nella cartella e si lascia lavorare il portale, con un nome file da fotocopiatrice (`SKM_C224e19061410250.png`) a dimostrare che nessuna convenzione serve. I più importanti sono quelli sulla prudenza: per default **nessuna presenza viene registrata**, chi non ha firmato non viene toccato, l'automatismo si ferma se non tutti risultano firmati, un foglio non riconosciuto finisce in `errori\` **ma non si perde** (resta archiviato), niente viene letto due volte, i file estranei sono ignorati, il limite per giro è rispettato, la cartella irraggiungibile non è un guasto, e a spenta non si guarda nemmeno. Più 6 test sulla pagina impostazioni. Suite `anagrafica` + `core`: **1571 test**, nessuna regressione.
...
### Formazione — catena dell'evidenza: origine dell'obbligo sul corso + registro audit dei gesti irreversibili

- **[feat/test] `requirements.in`, `requirements.txt` (`zxing-cpp==3.1.1`), `core/qr.py` [nuovo], `core/tests_qr.py` [nuovo]**: **lettura dei codici QR** dai documenti scansionati. Il foglio firme stampava già un QR col token, ma nessuno lo leggeva: il codice andava **ribattuto a mano** sotto il codice stesso. Aggiunta la dipendenza `zxing-cpp` (wheel binario `win_amd64` da 1 MB, **nessuna libreria di sistema** da installare a parte, decodifica QR *e* codici a barre lineari) e scritto `core.qr` — deliberatamente in `core` e **non** nella formazione, perché il meccanismo (*stampo un documento con un codice, torna indietro scansionato, il portale riconosce da solo di quale documento si tratta*) non ha niente di specifico e vale identico per una scheda di sicurezza controfirmata, un verbale, una checklist di reparto. API minima: `disponibile()`, `leggi_codici()`, `leggi_codice(prefisso=…)`. **Non solleva mai**: un documento senza codice, storto, sbiadito, corrotto o un ambiente in cui la libreria non è ancora installata restituiscono lista vuota — chi chiama deve sempre avere la strada alternativa, perché un lettore di codici è una comodità, non un requisito. Accetta bytes (PDF rasterizzato o immagine) oppure un'immagine PIL già aperta, così chi ha già rasterizzato per altri motivi non lo rifà; sui PDF ritenta a 300 dpi se a 200 non trova nulla. **10 test**: QR da PNG, da PDF, da PIL diretto, su foglio ruotato di 6°, foglio bianco, file corrotto, filtro per prefisso, e la degradazione a vuoto quando la libreria manca.
- **[feat/test] `anagrafica/views.py` (`formazione_registro_scansione`), `anagrafica/templates/anagrafica/pages/formazione_presenze.html`, `anagrafica/templates/anagrafica/pages/formazione_scansione_esito.html`**: il foglio firme **si riconosce da sé**. Caricando la scansione non si indica più nulla: il token si decodifica dal QR e viene risolto **per corrispondenza sul database** (non per prefisso), il che valida e identifica in un colpo solo. Il campo del codice **resta**, ora facoltativo, perché un QR strappato, piegato o macchiato capita e la strada a mano costa zero. Invariato il controllo che conta: il foglio deve appartenere a **questa** giornata, altrimenti le presenze finirebbero sul giorno sbagliato.
- **[feat/test] `anagrafica/models_formazione.py` (`TrainingScanLog`), `anagrafica/migrations/0103_trainingscanlog.py` [nuovo], `anagrafica/services/archivio_scansioni.py` [nuovo], `anagrafica/views.py` (`formazione_scansioni_log`, `formazione_scansione_scarica`), `anagrafica/urls.py`, `anagrafica/templates/anagrafica/pages/formazione_scansioni_log.html` [nuovo]**: **registro delle letture**, nato dal caso in cui la lettura **fallisce**. Prima il file caricato spariva e restava solo un messaggio d'errore a schermo: chi aveva scansionato non aveva niente da mostrare e chi doveva capire il perché non aveva niente da guardare. Ora il file viene archiviato **prima ancora del tentativo di lettura**, riuscita o fallita che sia, e il registro dice **dove**: nuova pagina `/anagrafica/formazione/scansioni/registro/` con esito, messaggio d'errore nelle stesse parole mostrate all'utente, percorso di archiviazione, dimensione, codice (distinguendo *letto dal QR* da *digitato*), giornata, misure della lettura e inclinazione rilevata; filtri per esito e ricerca per file/codice/corso. Archivio nello **storage privato dell'anagrafica** — lo stesso di referti e consegne, fuori webroot e cifrato a riposo — perché un foglio firme è un elenco di nomi con le firme autografe: i file si riaprono dalla pagina, dove il download applica permessi e lascia traccia in `AuditLog`, non da Esplora risorse. Percorso ordinato per anno/mese, nome del file ripulito (uno scanner di rete produce nomi con spazi, accenti e percorsi interi). Sia l'archiviazione sia la scrittura della riga sono **fail-safe**: un guasto del registro non deve far perdere il lavoro di chi sta caricando.
- **[fix] `anagrafica/templates/anagrafica/pages/formazione_scansione_esito.html`, `anagrafica/templates/anagrafica/pages/formazione_scansioni_log.html`**: le due pagine nuove chiamavano `messages.*` senza includere `flash_messages.html`, quindi gli errori di lettura e di download **non arrivavano a schermo**. Rilevato in fase di merge dalla guardia introdotta con le 8 migliorie trasversali, che è esattamente il suo mestiere.
- **[test] `anagrafica/tests_formazione_scansioni_log.py` [nuovo]**: **14 test**, i più importanti dei quali falliscono apposta — il foglio riconosciuto dal QR senza digitare niente, il codice a mano che resta valido, il foglio di un'altra giornata rifiutato, il file archiviato e **riapribile identico** anche quando la lettura è fallita, il percorso ordinato per anno/mese, la ripulitura del nome (compreso `../../etc/passwd` → `passwd`), la pagina che elenca e filtra, il download del file archiviato, un file sparito che non rompe la pagina, e il non-editor bloccato. Suite formazione completa (16 moduli) + `core.tests_qr`: **274 test** verdi.
- **[feat/test] `anagrafica/services/lettura_foglio_firme.py` [nuovo], `anagrafica/views.py` (`formazione_registro_scansione`), `anagrafica/urls.py`, `anagrafica/templates/anagrafica/pages/formazione_scansione_esito.html` [nuovo], `anagrafica/templates/anagrafica/pages/formazione_presenze.html`, `anagrafica/tests_formazione_lettura_firme.py` [nuovo]**: **lettura della scansione** del foglio firme, seconda metà dell'anello 6. Dalla pagina presenze si carica la scansione (PDF o immagine) indicando il token stampato sotto il QR; il servizio rasterizza, individua i **quattro marcatori d'angolo**, **raddrizza** il foglio se inclinato oltre 0,25°, costruisce la corrispondenza millimetri→pixel e misura quanto inchiostro contiene ogni cella. Nessun riconoscimento del testo: **non serve leggere le firme, serve vedere se ci sono** — e la posizione delle celle è già registrata dalla generazione. Soglia adattata al fondo effettivo della scansione (una fotocopia grigia e una scansione brillante hanno fondi lontanissimi) e **banda di incertezza**: le celle con segno troppo debole sono marcate «dubbie» ed evidenziate perché qualcuno le guardi. L'esito è una **proposta**: la pagina di conferma pre-spunta le caselle e le invia all'autocompilazione **già esistente e collaudata**, così non nasce una seconda strada di scrittura. La presenza a un corso è un atto con valore legale: la conferma resta di una persona, non di una misura di pixel. Lettura tracciata in `AuditLog`. Nessuna dipendenza nuova.
- **[test] `anagrafica/tests_formazione_lettura_firme.py` [nuovo]**: 22 test costruiti su un **giro completo** — si emette il foglio, lo si rasterizza come farebbe uno scanner, ci si «firma» sopra disegnando dentro alcune celle e si rilegge: devono tornare esattamente quelle righe. Copre i casi che rompono davvero uno scanner (foglio ruotato di 1,2°, risoluzioni 150/200/300 dpi, PDF invece di immagine, marcatori tagliati fuori, file illeggibile, immagine minuscola), il **falso positivo** su foglio intonso, l'indipendenza fra ingresso e uscita, la posizione reale dei marcatori, e sul lato endpoint: nessuna scrittura in fase di lettura, token in minuscolo accettato, **token di un'altra giornata rifiutato**, non-editor bloccato, tracciamento in audit.
- **[feat/test] `anagrafica/models_formazione.py` (`TrainingSignatureSheet`), `anagrafica/services/foglio_firme.py` [nuovo], `anagrafica/views.py` (`formazione_lezione_registro_qr`), `anagrafica/urls.py`, `anagrafica/templates/anagrafica/pages/formazione_sessione_detail.html`, `anagrafica/migrations/0102_trainingsignaturesheet.py` [nuovo], `anagrafica/tests_formazione_foglio_firme.py` [nuovo]**: **foglio firme tracciato** (anello 6), prima metà del lavoro sulle presenze. Il registro cartaceo resta l'unico documento che un ispettore accetta senza discutere: l'obiettivo è farlo diventare **anche il modo di compilare il portale**. Nuovo pulsante «▣ Foglio con QR» accanto a quello classico (che **resta invariato**): emette un foglio A4 con **QR contenente il token**, **quattro marcatori d'angolo** e il token in chiaro sotto il QR (se il codice si rovina, si ribatte a mano — l'alfabeto esclude `0/O` e `1/I`). Ogni emissione **registra** un `TrainingSignatureSheet` che **congela l'elenco**: se domani si aggiunge un iscritto, la riga 7 della scansione resta la persona di allora, e una ristampa emette un foglio **nuovo** perché due fogli con lo stesso token porterebbero elenchi diversi. Viene salvata anche la **geometria delle celle firma** in millimetri dall'angolo in alto a sinistra (come si ragiona su un'immagine, non come vuole il PDF), con `legacy_id` su ogni cella. La tabella è disegnata a coordinate fisse invece che con un flowable: un layout che «scorre» non garantirebbe la posizione al millimetro. Nessuna dipendenza nuova — `qrcode`, `reportlab`, `pymupdf` e `pillow` erano già in `requirements.txt`. **La seconda metà** (lettura della scansione) userà queste stesse coordinate: sapere chi ha firmato non richiede di *leggere* la firma, ma di misurare se un rettangolo di posizione nota contiene inchiostro.
- **[test] `anagrafica/tests_formazione_foglio_firme.py` [nuovo]**: 18 test — alfabeto del token senza caratteri ambigui, congelamento dell'elenco (iscritto aggiunto dopo che non altera il foglio emesso, ristampa che emette un token nuovo), lezione senza iscritti, geometria (una cella per firma per iscritto, coordinate dentro pagina e misurate dall'alto, righe che scendono, ingresso e uscita che non si sovrappongono, schema versionato), contenuto del PDF (token leggibile, quattro marcatori d'angolo), endpoint (emissione tracciata in `TrainingExportLog`, non-editor bloccato, lezione di un'altra edizione che dà 404).
- **[feat/test] `anagrafica/models_formazione.py` (`TrainingEnrollment.MODALITA_VERIFICA_CHOICES`, `modalita_verifica`, `punteggio`, `punteggio_minimo`), `anagrafica/forms.py` (`TrainingEnrollmentEditForm`), `anagrafica/migrations/0101_trainingcompletionrule_valutazione_efficacia_mesi_and_more.py` [nuovo]**: **evidenza della verifica di apprendimento** (anello 7). Per l'e-learning il quiz conservava punteggio, tentativi e risposte; per l'**aula** restava solo `verifica_superata`, un segno di spunta — che non dimostra un apprendimento. L'iscrizione porta ora **modalità** (test scritto · colloquio orale · prova pratica · osservazione sul campo), **punteggio** e **soglia applicata**: la soglia si scrive sulla riga, così l'esito resta rileggibile anni dopo **con il criterio di allora** e non con quello vigente al momento della lettura. Il form **deduce** `verifica_superata` quando ci sono punteggio e soglia (lasciarlo a mano significa ritrovarsi «superata» con 40 su 60) e propone come soglia iniziale quella della regola del corso.
- **[feat/test] `anagrafica/models_formazione.py` (`TrainingEfficacia`, `TrainingCompletionRule.valutazione_efficacia_mesi`), `anagrafica/services/formazione_efficacia.py` [nuovo], `anagrafica/views.py` (`_post_completamento`)**: **valutazione di efficacia** (anello 8), il requisito che ISO 45001 §7.2 e ISO 9001 §7.2 chiedono per nome e che nel modulo non esisteva affatto. Il corso dichiara **dopo quanti mesi** va verificato sul campo se la formazione ha prodotto competenza (`0` = non richiesta); al completamento nasce una valutazione **attesa** e datata, che scade e va compilata dal preposto con esito (efficace · parzialmente · non efficace), **azione conseguente** e note. È una **FK e non un uno-a-uno** di proposito: dopo un esito non pieno si concorda un'azione e si **rivaluta**, e le due valutazioni restano entrambe perché entrambe sono storia. L'aggancio è in `_post_completamento`, il punto unico attraversato da **tutte** le strade del completamento (aula, e-learning, registrazione diretta), ed è fail-safe: un errore qui non annulla mai un completamento. Servizio con `aggiungi_mesi` (31 gennaio + 1 mese = 28 febbraio, non un errore), `pianifica_valutazione_efficacia` (idempotente; non apre nulla su un esito non idoneo, perché non c'è efficacia da misurare) e `valutazioni_da_fare` (solo ciò che è **già dovuto**: un elenco che anticipa le scadenze future si ignora in fretta).
- **[feat] `anagrafica/services/attestato_pdf.py`**: il fascicolo mostra la verifica con **punteggio, soglia e modalità** invece del solo esito, e la sezione completezza guadagna la riga «Valutazione di efficacia sul campo» — **solo se il corso la prevede**, altrimenti sarebbe un rimprovero per un adempimento che nessuno ha chiesto.
- **[test] `anagrafica/tests_formazione_efficacia.py` [nuovo]**: 22 test — conservazione di punteggio/soglia/modalità, esito dedotto (anche al pari della soglia) e manuale in assenza di punteggio; aritmetica dei mesi con giorno inesistente; pianificazione idempotente, saltata su non idoneo e su regola disattivata; elenco delle pendenze ordinato e filtrato per persona, che esclude il futuro e il compilato; ciclo di **rivalutazione** dopo esito non pieno; resa nel fascicolo. Suite formazione completa (13 moduli, **210 test**) verde.

- **[feat/test] `anagrafica/models_formazione.py` (`TrainingCourse.FONTE_OBBLIGO_CHOICES`, `fonte_obbligo`, `riferimento_fonte`, `articolo_fonte`), `anagrafica/migrations/0099_trainingcourse_articolo_fonte_and_more.py` [nuovo], `anagrafica/forms.py` (`TrainingCourseForm`), `anagrafica/templates/anagrafica/pages/formazione_corso_form.html`, `anagrafica/tests_formazione_catena.py` [nuovo]**: **da dove nasce l'obbligo** di erogare un corso non era un dato ma testo libero dentro il titolo (`"… Rif. 9070Q"`, `"… AWPS004Q rev. B"`), quindi non interrogabile: in verifica ispettiva la domanda «mostrami tutti i corsi che discendono dall'Accordo Stato-Regioni» non aveva risposta. Aggiunti tre campi sul corso — `fonte_obbligo` (`LEGGE` · `ACCORDO` · `CLIENTE` · `NORMA` · `INTERNO`, indicizzato), `riferimento_fonte` (estremi della fonte) e `articolo_fonte` (punto specifico) — esposti nella stazione «Classificazione» del wizard corso. **Deliberatamente non bloccanti**: `blank=True, default=""` e `required=False` sul form, perché i 488 corsi già in catalogo non li hanno e un vincolo li avrebbe resi tutti invalidi. Migrazione puramente additiva.
- **[feat/test] `anagrafica/views.py` (`_audit_formazione`, `formazione_corso_delete`, `formazione_sessione_delete`, `formazione_lezione_delete`, `formazione_presenza_set`)**: il modulo non lasciava **traccia di chi avesse modificato cosa** — se una presenza veniva ritoccata dopo la firma o una sessione cancellata, non restava nulla. Agganciato il registro audit già esistente del portale (`core.audit.log_action`, modulo `anagrafica.formazione`) ai soli gesti che in audit sollevano la domanda «chi e quando»: cancellazione di corso, sessione (con conteggio delle lezioni travolte) e lezione (con le presenze), più la **modifica di una presenza esistente** quando ne cambia lo stato *oppure* quando la riga risultava già `FIRMATO`. Le registrazioni ordinarie **non** vengono tracciate di proposito: sono migliaia e renderebbero il registro illeggibile. Chiamata fire-and-forget: un errore di audit non fa mai fallire l'operazione dell'utente.
- **[feat/test] `anagrafica/models_formazione.py` (`TrainingCourseArgomento`, `TrainingSessionArgomento`, `TrainingLesson.argomenti_svolti`), `anagrafica/migrations/0100_trainingcourseargomento_trainingsessionargomento_and_more.py` [nuovo], `anagrafica/services/formazione_pianificazione.py` (`copia_programma_dal_corso`, `crea_sessione_unica`, `dividi_in_gruppi`), `anagrafica/views.py` (`formazione_sessione_create`), `anagrafica/tests_formazione_programma.py` [nuovo]**: **programma didattico** su due livelli. Per la formazione dei lavoratori i contenuti minimi sono normati e l'unico campo disponibile era `TrainingLesson.argomento`, una riga di testo per giornata: impossibile dimostrare di aver coperto il programma. Ora il **corso** dichiara il programma previsto (`ordine` · `argomento` · `ore_previste` · `riferimento` alla fonte, es. «Allegato A, punto 3») e l'**edizione** ne riceve una **copia — non un collegamento**: se il corso cambia programma mesi dopo, l'edizione già erogata continua a documentare com'era allora (stessa logica degli snapshot di docente/titolo, e `origine` è `SET_NULL` così cancellare la voce del corso non svuota lo storico). L'edizione può **integrare** argomenti non previsti (`aggiunto=True`): `copia_programma_dal_corso()` è idempotente e, anche con `forza=True`, **non butta mai le integrazioni** perché sono lavoro umano. La copia scatta alla creazione della sessione (view e `crea_sessione_unica`); `dividi_in_gruppi` fa ereditare ai gruppi il programma **della sorgente**, integrazioni comprese, perché sono la stessa erogazione divisa. Il collegamento `TrainingLesson.argomenti_svolti` (M2M) registra quali voci sono state coperte in ciascuna giornata — è ciò che rende confrontabili previsto ed erogato.
- **[feat/test] `anagrafica/views.py` (6 endpoint programma + contesto `formazione_corso_detail`/`formazione_sessione_detail`), `anagrafica/urls.py`, `anagrafica/templates/anagrafica/pages/formazione_corso_detail.html`, `anagrafica/templates/anagrafica/pages/formazione_sessione_detail.html`, `anagrafica/tests_formazione_programma_ui.py` [nuovo]**: **editor in-app del programma**, che prima si compilava solo dall'area amministrativa. Card **«📚 Programma didattico»** sulla scheda corso (aggiunta con ordine progressivo automatico, ore con virgola decimale accettata, riferimento alla fonte, rimozione che **non tocca le edizioni già erogate**) e card **«📚 Programma dell'edizione»** sulla scheda sessione con il badge dei *non svolti*, il pulsante **«↻ Riprendi dal corso»** (che conserva le integrazioni) e l'integrazione di argomenti non previsti (marcati `aggiunto`). La **copertura** si segna dalla riga dell'argomento spuntando le giornate — direzione argomento→giornate, che evita di toccare la tabella delle lezioni ed è il verso in cui si ragiona davvero («dove ho trattato questo contenuto?»). L'endpoint accetta **solo giornate dell'edizione corrente**: un id arrivato da fuori viene scartato.
- **[test] `anagrafica/tests_formazione_programma_ui.py` [nuovo]**: 14 test — ordine progressivo, argomento vuoto e ore non numeriche rifiutati senza rompere, rimozione dal corso che lascia intatte le edizioni, «Riprendi dal corso» che conserva le integrazioni, copertura impostata/azzerata, **giornata di un'altra edizione ignorata**, non-editor bloccato, e la resa in pagina di entrambe le schede.
- **[feat] `anagrafica/services/attestato_pdf.py` (`build_fascicolo_sessione_pdf_bytes`)**: il fascicolo espone la nuova sezione **«Programma dichiarato e copertura»** (argomento · ore · riferimento · *in quali giornate è stato svolto*, oppure `NON svolto`), e la sezione completezza guadagna la voce «Programma coperto dalle giornate» col conteggio degli argomenti scoperti — o «Programma didattico dichiarato: Mancante» se il programma non c'è affatto.
- **[test] `anagrafica/tests_formazione_programma.py` [nuovo]**: 12 test — la copia è davvero una copia (modificare il corso non tocca l'edizione, cancellarne una voce non svuota lo storico), idempotenza, conservazione delle integrazioni in ricopiatura forzata, programma nella sessione unica e nei gruppi, copertura argomento↔giornata e le due righe che il fascicolo ne deriva.
- **[feat/test] `anagrafica/services/attestato_pdf.py` (`build_fascicolo_sessione_pdf_bytes`), `anagrafica/tests_formazione_fascicolo.py` [nuovo]**: il **fascicolo dell'edizione** esisteva già (progettazione · programma · partecipanti · relazione) ma rispondeva al «cosa» senza rispondere al «perché» né al «come lo dimostri». **Non è stato rifatto: è stato completato.** (1) La testata ora parte dal **piano formativo** e dichiara l'**origine dell'obbligo** (fonte + estremi + articolo, dai nuovi campi) e la **qualifica rilasciata** quando presente — prima il documento cominciava dal corso, cioè a metà catena. (2) Nuova sezione **«Evidenza delle presenze»**: una riga per giornata con attesi, presenti, quante firme risultano e se il **registro firmato è allegato** (per giornata, a livello edizione, oppure `NON allegato` detto esplicitamente). (3) Nuova sezione **«Completezza del fascicolo»**: l'unica parte del documento che dichiara **ciò che manca** — origine dell'obbligo, argomento su ogni giornata, registro firmato, docente, esito per iscritto e verifica finale quando il corso la richiede — ciascuno con il conteggio (`Mancante su 2 di 5`). Serve a scoprire i buchi *prima* che li chieda un ispettore. Nessuna migrazione, nessun nuovo endpoint: stesso PDF, stessa URL.
- **[test] `anagrafica/tests_formazione_fascicolo.py` [nuovo]**: 11 test sul **testo estratto** dal PDF (pymupdf, già in requirements) e non sull'impaginazione, che resta libera di cambiare: testata con piano e fonte, generazione corretta anche senza fonte (corsi storici), conteggio presenze/firme, registro non allegato dichiarato, sezione completezza con i conteggi dei mancanti, sessione senza giornate né iscritti che non rompe. Suite formazione completa (10 moduli, **162 test**) verde.
- **[test] `anagrafica/tests_formazione_catena.py` [nuovo]**: 13 test — il corso resta valido senza fonte (non bloccante), la fonte è interrogabile per categoria, i valori fuori catalogo sono rifiutati, il form salva/omette correttamente; sul lato audit: la prima registrazione di presenza **non** sporca il registro, il cambio di stato su presenza esistente sì, il ritocco dopo la firma è tracciato anche a stato invariato, e ogni voce riporta l'operatore. Suite formazione completa (9 moduli, 151 test) verde, nessuna regressione.
...
- **[fix] `anagrafica/forms.py` (`AnagraficaCivileForm`, `AnagraficaAziendaleForm`)**: aprendo «✏ Modifica» su «Anagrafica civile»/«Anagrafica aziendale», i campi data (`data_nascita`, `data_consenso_privacy`, `data_prima_assunzione`, `data_assunzione_ultima`, `data_cessazione`, `prova_data_inizio`, `prova_data_fine`) apparivano vuoti pur avendo un valore salvato (visibile nella vista di sola lettura). Causa: i widget `DateInput(type="date")` non specificavano `format="%Y-%m-%d"`, quindi Django renderizzava il valore in formato locale (`gg/mm/aaaa`) — non riconosciuto dall'input HTML5 `type="date"`, che il browser mostra vuoto. Stesso trap già noto e corretto in questo file per `FiglioACaricoForm.data_nascita`, non applicato a questi due form. Aggiunto `format="%Y-%m-%d"` a tutti e sette i widget.

### Registro OFI — colonna «STATO PDCA» compatta in testa alla tabella

- **[feat/test] `gestione_specifiche/templates/registro_ofi/lista.html`, `gestione_specifiche/tests/test_registro_ofi.py`**: le colonne P/D/C/A del MOD.174 sono **in fondo** alla tabella (dopo ~17 colonne), quindi invisibili senza scorrere in orizzontale; l'utente le vuole a colpo d'occhio. Aggiunta una colonna **«STATO PDCA»** compatta **subito dopo «OFI/NC»** (in testa, sempre visibile) con un **indicatore a 4 segmenti P·D·C·A** che mostra **tutte e quattro le tappe** insieme, con i **colori fissi per lettera del MOD.174**: **P azzurro · D rosso · C giallo · A verde** (classi `ofi-k-{p,d,c,a}`). Tappa **raggiunta** = colore pieno (`ofi-mini-done`/`ofi-mini-current`, con la corrente cerchiata d'arancio); tappa **da fare** = tinta chiara della stessa lettera (`ofi-mini-todo`) — riusa la property `pdca_steps`. Le colonne MOD.174 in fondo (P·D·C·A·TOT, X cumulative) **restano invariate** per fedeltà al foglio. Test aggiornato (`test_lista_indicatore_pdca_compatto_in_testa`: 4 segmenti, 2 done + 1 current + 1 todo, più i colori per-lettera P azzurro/D rosso/C giallo/A verde su una voce in CHECK); suite `test_registro_ofi` 35 verdi. Nessuna migrazione.

### Registro OFI — lista full-width, PDCA cumulativo come l'Excel, badge OFI/NC e «Nuovo inserimento»

- **[feat/style/test] `gestione_specifiche/models.py` (`RegistroOFI.pdca_steps`), `gestione_specifiche/templates/registro_ofi/{lista,form,dettaglio}.html`, `gestione_specifiche/tests/test_registro_ofi.py`**: correzioni UI del Registro OFI segnalate dall'utente dopo l'import del MOD.174. (1) **Lista a tutta larghezza**: la tabella MOD.174 (min-width `1560px`) veniva compressa dal cap `.content{width:min(100%,1680px)}` di `theme.css`, lasciando le colonne P/D/C/A/TOT in overflow non visibile; ora la pagina lista **override-a il cap** (`.content{width:100%;max-width:100%}`, solo per questa pagina). (2) **Colonne P/D/C/A cumulative come l'Excel**: le celle mostrano di nuovo **tutte le X cumulative** (una voce in CHECK ha la X in P, D e C — verde su tappa raggiunta, grigio su tappa da fare), **non** solo la fase corrente; rimossa la colonna «FASE» a stato singolo e l'evidenza chip-arancio introdotte nell'iterazione precedente. Resta la colonna **TOT** con la lettera di fase e, nel **dettaglio**, il pannello **«Stato PDCA»** con stepper P→D→C→A (che continua a usare `pdca_steps`). (3) **Badge tipo a colori semantici**: **OFI = giallo/ambra** (`fmd-b-amber`), **NC = rosso** (`fmd-b-red`), in lista e dettaglio. (4) **Inserimento neutro OFI/NC**: il bottone «Nuovo OFI» diventa **«Nuovo inserimento»** e il form apre con la **scelta OFI/NC** come primo campo in evidenza (full-width + help); titoli/label del form resi neutri («Nuovo inserimento», «Modifica voce», «Crea voce»). Nessun campo/nessuna migrazione (stato PDCA sempre derivato da `fase`). 4 test aggiornati/nuovi (PDCA cumulativo in lista, badge OFI-ambra/NC-rosso, bottone «Nuovo inserimento», form neutro con scelta OFI/NC, badge OFI-ambra nel dettaglio); suite `test_registro_ofi` 34 verdi + `test_mod174_import` 7 verdi.

### Assets — bottone «Assegna progressivo» prefix-aware (convenzione `Int.NNN`)

- **[feat/test] `core/numbering.py` (`next_suffix`/`next_code` con `sep`/`pad`, nuovo `next_prefixed`), `core/tests_numbering.py`, `assets/views.py` (`asset_internal_number_next` + costanti `INTERNAL_NUMBER_PREFIX`/`SEP`/`PAD`), `assets/tests.py`**: l'endpoint `assets:internal_number_next` (bottone «Assegna progressivo» del form asset) ora segue la **convenzione storica di produzione `Int.NNN`**: propone `Int.263` (= max `Int.262` + 1) invece del nudo `198`. Il nuovo helper puro `next_prefixed(values, prefix, sep, pad)` tratta come **un'unica sequenza numerica** sia i codici già prefissati (coda numerica dopo `Int.`) sia i **valori nudi legacy** senza prefisso (es. i due anomali `196`/`197`), ignorando le code non interamente numeriche (`Int.188A`, `Int.55/B`) ed emettendo **sempre** il prefisso con zero-padding a 3 cifre. `next_suffix`/`next_code` guadagnano i parametri opzionali `sep` (default `-`) e `pad` (default `0`), **retro-compatibili** col chiamante esistente in `anagrafica` (codice corso `<piano>-<N>`). 7 nuovi/adeguati test (5 `next_prefixed` + 2 endpoint asset); `next_numeric`/`max_numeric` invariati.

### Registro OFI — modulo standalone `/ofi-registro/` (MOD.174), inserimento utente + fullscreen

- **[feat/test] `config/urls.py`, `gestione_specifiche/urls_ofi.py` [nuovo], `gestione_specifiche/views_ofi.py` [nuovo], `gestione_specifiche/urls.py`, `gestione_specifiche/views.py` (rimossa `ofi_registro`), `gestione_specifiche/forms.py` (`RegistroOFIForm`), `gestione_specifiche/models.py` (`RegistroOFI.pdca_marks`/`tot_letter`), `gestione_specifiche/registro_ofi.py` (`conta_pdca_cumulativo`), `gestione_specifiche/acl_bootstrap.py` (`PERM_OFI_ADD`, binding `registro_ofi:*`, cache key v17), `gestione_specifiche/templates/registro_ofi/{lista,form,dettaglio}.html` [nuovi], rimosso `gestione_specifiche/templates/gestione_specifiche/ofi_registro.html`, `gestione_specifiche/templates/gestione_specifiche/lista.html` (link), `core/migrations/0070_ofi_registro_toplevel_nav.py` [nuovo], `gestione_specifiche/tests/test_registro_ofi.py`**: il Registro OFI diventa un **modulo trasversale standalone** montato al **top-level `/ofi-registro/`** (namespace `registro_ofi`), non più sotto `gestione-specifiche`. Gestione Specifiche lo **richiama con filtro** (`?modulo=gestione_specifiche`), non lo possiede (il modello resta in `gestione_specifiche` per continuità con MOD.133/`AzioneOFI`). La lista è **a tutta larghezza** (fedele all'Excel, niente layout centrato) e **replica esattamente il MOD.174 SGI**: intestazione «Costruzioni Novicrom — MOD.174 – SGI Registro OFI/NC Rev.0», colonne REF · DATA · OFI/NC · Normative (27001/45001/9100) · REF NORMA · PROCESSO · OPPORTUNITY · PLAN · Allegato/Link · DO · CHECK · ACT · DATA REQUIRED · DATA CLOSED · OWNER · **P·D·C·A·TOT** (X cumulative + lettera fase derivate da `fase`), contatori **cumulativi** P/D/C/A e **KPI ≥90%** (A/P) come la riga 2 del foglio. Nuovo **inserimento/modifica utente** (`RegistroOFIForm`, tutti i campi MOD.174) gated dal permesso canonico **`gestione_specifiche.registroofi.add`** (pulsante «+ Nuovo OFI»/«✎» mostrati solo a chi ha il permesso; grant di default per admin/amministrazione/qualità). Scheda `dettaglio` di sola lettura. La voce topbar «Registro OFI» ora punta a `registro_ofi:lista`.
- **[feat/test] `gestione_specifiche/forms.py` (`RegistroOFIForm.modulo_origine`), `gestione_specifiche/views_ofi.py` (`moduli_esistenti` nel context di `nuovo`/`modifica`), `gestione_specifiche/templates/registro_ofi/form.html` (campo + `<datalist>`)**: il form di inserimento/modifica ha ora il campo **«Modulo di competenza»** a **testo libero** con **datalist** che propone i moduli già presenti (chiave→etichetta da `moduli_presenti()`). Così anche le OFI inserite a mano risultano **attribuite a un modulo** e quindi filtrabili nel registro unico (colonna «Modulo» + filtro). Vuoto = solo registro unico.
- **[style] `gestione_specifiche/templates/registro_ofi/{lista,form,dettaglio}.html`, `gestione_specifiche/forms.py` (`RegistroOFIForm` widget senza classe)**: restyle delle 3 pagine OFI nello **stile del modulo Anagrafica** (design-system canonico portale-wide). Riuso del guscio `fmd-` (`anagrafica/css/formazione_design.css` + `_hr_restyle.html`), del form-kit `hub-` (`core/components/_hub_formkit.html`, `hub-form-stack`/`hub-fsec`/`hub-field`/`hub-pill`) e delle icone `_fm_icons.html`. Lista: **hero** con tile KPI (voci totali + % completamento), toolbar filtri `fmd-toolbar`, tabella `fmd-table` con badge di stato (`fmd-b-*`), legenda PDCA a pill; form: sezioni verticali `hub-fsec` con campi `hub-field`; dettaglio: pannelli `fmd-block` a definition-list. Fix collaterale: rimosso il parametro `modulo` duplicato (hidden+select) nella toolbar. Solo presentazione, logica invariata.
- 9 nuovi test registro (URL top-level, replica MOD.174 + KPI, contatori cumulativi, inserimento/modifica, dettaglio, binding ACL, campo modulo + datalist); 327 test `gestione_specifiche` verdi. Nessuna migrazione di schema (solo property + data migration nav).

### Registro OFI — import di un MOD.174 già compilato (comando `import_mod174`)

- **[feat/test] `gestione_specifiche/mod174_import.py` [nuovo] (`leggi_righe_mod174`, `importa_voci`), `gestione_specifiche/management/commands/import_mod174.py` [nuovo], `gestione_specifiche/tests/test_mod174_import.py` [nuovo]**: nuovo comando **`manage.py import_mod174 <file.xlsx>`** per **caricare un MOD.174 SGI già compilato** (l'Excel storico delle OFI) nel registro centralizzato. Il parser legge il layout canonico del foglio (intestazione «REF» individuata automaticamente, colonne A→S; la **colonna TOT è una formula e viene ignorata**), salta le righe pre-numerate vuote e **deriva la fase PDCA dalle X** di P/D/C/A (1→PLAN … 4→ACT; **CHIUSO** se «DATA CLOSED» è valorizzata) — coerente con la colonna TOT del foglio. L'import è **idempotente**: upsert sul numero di registro (REF), quindi rilanciarlo aggiorna le voci esistenti senza duplicarle. Default **DRY-RUN** (nessuna scrittura, stampa cosa farebbe); `--apply` esegue davvero; `--modulo <chiave>` attribuisce le voci importate a un modulo d'origine (compare nella colonna «Modulo» del registro unico); `--sheet <nome>` sceglie il foglio. 7 test (parsing + ValueError su file non-MOD.174, dry-run/apply/idempotenza, comando con/senza `--apply`); nessuna migrazione. Il modulo di parsing è separato dal command per essere testabile senza I/O su disco.
...
### Epica B / 1.12 — Skill Matrix: gate qualificante I→L + contatore abilitati

- **[feat/test] `anagrafica/models_skillmatrix.py` (`CompetenzaSkm.corso_qualificante`), `anagrafica/migrations/0094_competenzaskm_corso_qualificante.py` [nuovo], `anagrafica/services/skillmatrix_qualifica.py` [nuovo], `anagrafica/views.py` (`skill_matrix_macchina`), `anagrafica/templates/anagrafica/pages/skill_matrix_macchina.html`, `anagrafica/admin.py` (`CompetenzaSkmAdmin`), `anagrafica/tests_skillmatrix_qualifica.py` [nuovo]**: (punto 1.12) **blocco I→L** — una macchina può dichiarare un **corso qualificante** (`CompetenzaSkm.corso_qualificante`); il servizio `skillmatrix_qualifica` valida che un livello ≥ **L (Intermedio)** sia consentito **solo** se il dipendente ha quel corso completato e valido (`TrainingDeadline`). `imposta_livello(..., forza=…)` è l'unico punto di scrittura: senza `forza` solleva se il gate non è soddisfatto, con `forza=True` applica **e registra l'eccezione** su `AbilitazioneMacchinaStorico` (append-only, EN 4179/Part 145 «training verificato»). Aggiunto il **contatore abilitati operativi per macchina** (nuova riga header nella matrice, ≥ soglia). Corso qualificante impostabile da admin (`CompetenzaSkm`). Migrazione additiva. 11 test.

### Epica B / 1.14 — MOD.128: requisiti multipli per qualifica (ampliamento 1-N)

- **[feat/test] `anagrafica/models_mpq.py` (`RequisitoQualifica` [nuovo modello]), `anagrafica/migrations/0093_requisitoqualifica.py` [nuovo], `anagrafica/services/mpq_conformita.py`, `anagrafica/forms_mpq.py` (`RequisitoQualificaForm`), `anagrafica/views_mpq.py` (`mpq_requisito_add/edit/delete`, `mpq_processo_detail`), `anagrafica/urls.py`, `anagrafica/acl_bootstrap.py`, `anagrafica/templates/anagrafica/pages/{mpq_processo_detail,mpq_requisito_form}.html`, `anagrafica/tests_mpq_requisiti_generici.py` [nuovo], `anagrafica/tests_mpq_crud.py`**: (punto 1.14) un processo qualificato (MOD.128) può ora dichiarare **N requisiti generici tipizzati** oltre ai tre M2M fissi (corsi/DPI/visite): `audit`, `corso`, `certificato`, `esame`, `esperienza`, `test visivo`, `DPI`, `riferimento normativo`, `altro`. Ogni requisito ha **stato/scadenza propri** (soddisfatto / da verificare / scaduto / N.A.), periodicità, riferimento normativo ed **evidenza allegabile** (link). Sono requisiti a **livello processo** (attestazione unica per tutti gli abilitati): la conformità (`verifica_requisiti`) li valuta insieme ai requisiti per-persona e l'esito degrada solo per i requisiti **obbligatori** (quelli non obbligatori sono informativi). CRUD in-app gated `anagrafica.mpq.manage`; storico append-only. Migrazione **additiva** (i tre M2M restano invariati). 13 test (7 modello/conformità + 5 CRUD + 1 esistente adeguato).

### Epica C / 4.2 — Registro OFI centralizzato PDCA (allineato MOD.174)

- **[feat/test] `gestione_specifiche/registro_ofi.py` (`MODULO_LABELS`, `modulo_label`, `moduli_presenti`), `gestione_specifiche/models.py` (`RegistroOFI.modulo_display`), `gestione_specifiche/views.py` (`ofi_registro`), `gestione_specifiche/templates/gestione_specifiche/{ofi_registro,lista}.html`, `core/migrations/0069_ofi_registro_nav.py` [nuovo], `gestione_specifiche/tests/test_registro_ofi.py`**: registro reso **universale/trasversale** e **raggiungibile**. Nuova **voce topbar «Registro OFI»** (categoria «Qualità / SGI», permesso ereditato dalla rotta `gestione_specifiche.specifica.view`, ACL-aware) → **registro unico** di tutti i moduli. La vista accetta `?modulo=<chiave>`: nessun filtro = registro unico; con filtro = **registro del singolo modulo** (contatori/scaduti coerenti col filtro). Colonna «Modulo» nel registro unico ed etichette leggibili (`MODULO_LABELS`, estendibile). Link **«Registro OFI»** nell'hero del modulo Specifiche che apre il proprio registro (`?modulo=gestione_specifiche`). Oggi solo MOD.133 genera OFI; la FK generica + `modulo_origine` sono la fondazione pronta per altri moduli.
