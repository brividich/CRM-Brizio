...
### Registro OFI — lista a tutta larghezza + stato PDCA evidente

- **[feat/style/test] `gestione_specifiche/models.py` (`RegistroOFI.pdca_steps`), `gestione_specifiche/templates/registro_ofi/lista.html`, `gestione_specifiche/templates/registro_ofi/dettaglio.html`, `gestione_specifiche/tests/test_registro_ofi.py`**: due correzioni all'UI del Registro OFI segnalate dall'utente dopo l'import del MOD.174. (1) **Lista a tutta larghezza**: la tabella MOD.174 (21 colonne, `min-width:1560px`) veniva compressa dal cap `.content{width:min(100%,1680px)}` di `theme.css`, lasciando le colonne P/D/C/A/TOT in overflow non visibile; ora la pagina lista **override-a il cap** (`.content{width:100%;max-width:100%}`, solo per questa pagina) così tutte le colonne sono visibili. (2) **Stato PDCA evidente**: nuova **colonna «FASE»** parlante (pill Plan/Do/Check/Act/Chiuso a colori) subito dopo «OFI/NC», visibile **senza scorrere**; le celle P/D/C/A ora **evidenziano la fase corrente** (chip arancio) distinguendola dalle tappe già completate (X verde); nel **dettaglio** nuovo pannello **«Stato PDCA»** con **stepper P→D→C→A** (tappa completata/in corso/da fare) e badge «Fase attuale». Nuova property derivata `pdca_steps` (nessun campo/nessuna migrazione: stato ricavato da `fase`). 5 nuovi test (`pdca_steps` corrente/chiuso, lista full-width, evidenza fase in lista, stepper nel dettaglio); suite registro+import 37 test verdi.
