...
def periodic_check_list(request: HttpRequest) -> HttpResponse:
    ...
    if intake_pending:
        todo.insert(0, {
            "tone": "warn", "rank": 1, "kind": "Da smistare",
            "title": "Cartella scansioni",
            "detail": (f"{intake_pending} scansion{'e' if intake_pending == 1 else 'i'} senza QR riconosciuto: "
                       "assegnale alla loro verifica."),
            "url": reverse("assets:periodic_check_intake"), "action": "Smista",
        })
        todo.sort(key=lambda row: row["rank"])
    category_counts = {}
    for check_type in types:
