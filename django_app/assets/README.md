...
- Il centro canonico e `/assets/manutenzione/`, con accessi distinti a lavoro di oggi, scadenzario, interventi, storico aziendale e `Catalogo e piani`.
...
- Le pagine operative condividono la sotto-navigazione `Oggi`, `Scadenzario`, `Interventi`, `Storico`, `Catalogo e piani`, `Report`. L'azione globale `+ Nuovo intervento` apre `/assets/workorders/?create=1`: prima si sceglie l'asset obbligatorio, poi si compila il form OdL guidato. Anche `/assets/workorders/new/` senza asset converge sul selettore.
...
- Nel dettaglio del singolo asset, la tabella `Manutenzione pianificata` e il riquadro `Prossima manutenzione` cercano l'OdL aperto relativo alla regola: se esiste, `Compila e chiudi rapporto` porta subito al consuntivo formale; se il job automatico non lo ha ancora creato, `Genera rapporto` lo predispone una sola volta dal template effettivo e apre la stessa chiusura. Non si ripassa dal form generico `Nuovo intervento`.
