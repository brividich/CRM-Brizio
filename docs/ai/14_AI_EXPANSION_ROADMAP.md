...
| 6.3 | **✅ FATTO — Tool live Schede Sicurezza** ("quale scheda per prodotto X", "quali scadute", conteggio prese visione) | `ai_assistant/tools.py::_schede_sicurezza_context`; gate ACL v2 `schede_sicurezza.prodotto.view`, seed routing `"schede_sicurezza"`, 4 test | **Whitelist stretta applicata**: prodotto/reparto/versione/scadenza/CLP/DPI + prese visione come **CONTEGGIO**; `PresaVisioneScheda` nominativo → **MAI i nomi degli operatori**. Complementare al RAG SDS (6.2, contenuto) | Medio | **S** | Medio |
| 6.4 | **✅ FATTO (tool + copilota) — Suggestion Corner** (stato PDCA aggregato + copilota classificazione/PLAN) | **Tool**: `ai_assistant/tools.py::_suggestion_context` (gate ACL v2 `suggestion_corner.segnalazione.view`, scope `visible_segnalazioni`, seed routing `"suggestioni"`, 4 test). **Copilota**: **già presente nel modulo** — `suggestion_corner/ai.py` (sessione 9): `classifica_ai` (SMS Sì/No), `bozza_plan_ai`, `trova_simili`, endpoint gated SMS_TEAM (JSON 403), read-only | **Tool**: solo aggregati, MAI nomi/testo/cliente. **Copilota**: `_contesto_segnalazione` usa solo reparto/processo/testo → **nessun autore nel prompt** (anonimato-safe), "AI propone / operatore firma". Nessuna DPIA bloccante | Medio | M | **Medio-Alto** |
| 6.5 | **✅ FATTO — Estensione tool Assets: OdL interne/esterne + fornitore** | `ai_assistant/tools.py::_workorder_line` arricchito (interna/🏢 esterna + Fornitore + origine), `select_related("supplier")`; 2 test | eredita il gate ACL del tool assets | Basso-Medio | S–M | Basso |

**Ordine consigliato Ondata 6**: ~~6.1~~ ✅ → ~~6.2~~ ✅ → ~~6.3~~ ✅ → ~~6.4~~ ✅ → ~~6.5~~ ✅. **Ondata 6 COMPLETA.**
