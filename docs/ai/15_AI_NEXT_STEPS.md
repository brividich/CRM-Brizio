...
| **A2** | **Copilota Incidenti/RCA** ✅ **F1 backend (dev)** — propone tipo_evento (validato su `TipoEventoSicurezza`)/causa/**5-Why**/azioni correttive; endpoint `rilevazione-incidenti/api/copilota/` gated (**`API_ACL_GATE_PATHS`→modulo** + inner `_can_create`/`_can_manage_rspp`). Copilota (no `AiToolPrivacyReview`: non espone PII nuovo). **UI F1b FATTA** (card in `nuovo.html`, pre-compila 5-Why/note) | copilota (proposto=True, fail-safe) | ACL modulo + audit no-testo | 🥇 |

> **Nota tecnica (ACLMiddleware):** ogni **nuova route API** va mappata in `core/middleware.py::API_ACL_GATE_PATHS` verso una risorsa ACL bound, altrimenti con `ACL_STRICT_CANONICAL=True` viene **negata** ai non-superuser. A3 (`/api/anomalie/…`) era già coperto; A2 ha richiesto la mappa `/rilevazione-incidenti/api/ → /rilevazione-incidenti/`.
