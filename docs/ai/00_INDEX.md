...
| [CHECKLIST_REFACTORING_MANUTENZIONI.md](CHECKLIST_REFACTORING_MANUTENZIONI.md) | Checklist di avanzamento del refactoring manutenzioni (Piano → Applicazione → Occorrenza → OdL): mappatura vecchio/nuovo, decisioni prese, cosa è fatto, cosa resta (UI, reminder, dismissione contatori), trappole del repository, API del dominio. Autosufficiente per una nuova sessione. |
| [CHECKLIST_SISTEMA_GESTIONE_AUDIT.md](CHECKLIST_SISTEMA_GESTIONE_AUDIT.md) | Checklist di avanzamento del modulo `sistema_gestione`: SoA ISO 27001 (MOD.165, fatta), audit interni EN 9100 (MOD.034/035A/035B) e ISMS (MOD.171/168) da fare. Decisioni dell'utente, documenti sorgente SGI, incongruenze da segnalare, modelli/flussi/PDF/test previsti, deploy. Autosufficiente per una nuova sessione. |
| [VERIFICHE_PERIODICHE.md](VERIFICHE_PERIODICHE.md) | Manutenzione > Verifiche periodiche sugli impianti (luci di emergenza, quadri, terra, antincendio...): confini con Scadenze amministrative e con la legacy «Manutenzioni periodiche», modello `PeriodicCheck*`, metodi di acquisizione (checklist, verbale, planimetria a punti, misure), import dello storico, deploy. |
