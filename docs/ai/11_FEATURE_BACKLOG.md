...
| AN-AUTO1 | 🟡 | **Difetto ricorrente per P/N** | task orario `anomalie_escalation` | N anomalie sullo stesso P/N in X giorni → mail supervisori + lista RDC, una per P/N per finestra | `[x]` 2026-09-26 `anomalie/automazioni_service.py`, spenta di default, config UI. Migrate `anomalie 0010` |
| AN-AUTO2 | 🟡 | **RDC richiesto senza numero** | task orario | Promemoria dashboard CC/CAR (`anomalia_rdc_mancante`) + sezione nel resoconto giornaliero | `[x]` 2026-09-26, spenta di default |
| AN-AUTO3 | 🔵 | **Digest settimanale KPI** | task orario, lunedì all'ora del resoconto | Mail ai supervisori con KPI della settimana | `[x]` 2026-09-26, spenta di default |
| AN-AUTO4 | 🟡 | **OP completato** | flush modifiche (~5 min) + giro orario sugli OP delle ultime 48h | Mail a CC/CAR con link al Report OP | `[x]` 2026-09-26, spenta di default. Alternativa in codice ad AU43 (che resta pacchetto bozza) |
