...
- **Copiloti per-modulo** (l'AI propone, l'umano firma): oltre ai tool che leggono i dati, alcuni moduli hanno un copilota che **propone** contenuti da rivedere e firmare (nessun salvataggio automatico). Disponibili: **Ticket** (triage dal testo → categoria/priorità/incide-sicurezza/assegnatario dal team gestori + bozza di risoluzione; pulsante «Copilota AI» nel dettaglio gestione, endpoint `tickets/api/copilota/` gated gestori, valori validati e fail-safe, audit metadata-only) e **Gestione Specifiche/MOD.133** (pre-compilazione righe flow-down + TAG dal PDF).
- **Guida HTML al funzionamento dell'AI** (per utenti, autoconsistente): [docs/ai/GUIDA_AI.html](docs/ai/GUIDA_AI.html) — architettura on-premise (Ollama+TEI), RAG SGI citabile, tabella dei tool runtime, copiloti per-modulo, esempi di domande, limiti/privacy/governance e roadmap. Va mantenuta aggiornata a ogni nuova capacità AI.
