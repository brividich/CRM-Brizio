...
- **Foto dipendente**: caricabile dall'anagrafica civile in creazione o dalla scheda dipendente; usata come avatar nella lista `/anagrafica/dipendenti/` e nella testata scheda, con fallback grigio neutro se assente.
...
- **Notifica al "Salva ed esci"**: l'endpoint `POST /api/anomalie/notifica-op` (`api_notifica_op`) invia **una sola mail aggregata** a CC/CAR eseguendo on-demand la regola automazione AU51 (`au51-anomalia-creata-mail-action-op`, `operation_type: manual`) via `run_rule` — l'invio finisce nel run log ed è gestibile da `/automazioni/regole/`. Nessun trigger automatico su insert, nessun cooldown, nessun doppione. Service riusabile `send_anomalie_action_email()` + management command `test_mail_action` per test e2e da CLI. Package automazione: `au51_anomalia_creata_mail_action_capocommessa.automation_package.json`
- **Mail di conferma post-aggiornamento**: quando il CC/CAR aggiorna le anomalie (dal form mail-action o dalla pagina web) parte un riepilogo HTML (`send_anomalie_update_confirmation`) a operatore segnalante + CC/CAR + lista fissa configurabile (config liste `conferma_aggiornamenti`). Dalla pagina web: tasto **"Salva e notifica"** (invio immediato) + **fallback debounce 5 min** via modello `AnomaliaPendingNotification`, task django-q2 `run_anomalie_pending_notifications` e command `flush_anomalie_notifications` (se l'operatore dimentica, la mail parte comunque dopo l'inattività)
- **Toggle nel form mail-action**: "Aprire RDC?" e "Segnalare a cliente?" replicano le regole Power Apps della pagina web (prima opzione avanzamento dinamica, "Chiudere automatico" calcolato, "Segnalare" disabilita l'avanzamento). Default avanzamento alla creazione: **"In attesa"**
