...
| 24 | [`schede_sicurezza`](django_app/schede_sicurezza/) | Sicurezza | `/schede-sicurezza/` | Archivio schede dati di sicurezza (SDS) prodotti chimici: prodotto↔**mansioni di rischio** (più mansioni per prodotto), scheda versionata con ingestion PyMuPDF section-aware (pittogrammi/frasi H-P/primo soccorso/DPI/incompatibilità), M2M DPI obbligatori, QR verso vista mobile sintetica **pubblica senza login** (`uuid` non PK, contatore visite), download PDF pubblico da storage privato cifrato, presa visione HTMX idempotente per versione, cruscotto personale delle SDS da leggere, notifica in-app al cambio mansione (l'email al dipendente è responsabilità della regola automazioni **AU56**, non più inviata da codice), link **"conferma tutte"** (`/schede-sicurezza/da-leggere/conferma-tutte/`, login richiesto) che registra in un click la presa visione di tutte le SDS dovute e riapre il cruscotto — usato anche dalla mail AU56, lista a card per mansione con pittogrammi CLP disegnati (sprite SVG) e filtro per pericolo |
...
- **Nominativo in formato unico «Nome Cognome»**: in tutto il modulo la persona si legge `Luca Bova` — elenco dipendenti ed ex dipendenti, scheda, organigramma (albero, diagramma e popup), scadenzario, matrice competenze, conformità, report, stampe (scheda e libretto formativo), export XLSX/CSV/PDF ed email di promemoria. Prima l'ordine era `Cognome Nome` e il testo arrivava come era stato scritto in fase di inserimento (MAIUSCOLO dall'import massivo, misto dagli inserimenti a mano). Due pezzi: (1) la **normalizzazione a iniziali maiuscole** avviene in scrittura in `core/naming.py`, richiamata dall'unico chokepoint `upsert_anagrafica_dipendente` — quindi vale per form, import XLSX, recruiting e assegnazioni — e gestisce nomi composti, apostrofi (`D'ANGELO` → `D'Angelo`) e trattini; (2) la **visualizzazione** passa dai tag `{% nominativo %}` / `{% nominativo_iniziali %}` / `{% nominativo_sort %}` (`anagrafica_extras`). Gli **ordinamenti restano per cognome** (colonne delle liste e tendine dipendenti), così l'elenco non cambia sequenza. Per allineare le righe già in archivio: `python manage.py normalizza_nomi_dipendenti` (anteprima) e poi `--apply`
...
- **Import archivio HR TOOLS** (`python manage.py importa_archivio_hr <cartella> [--apply] [--mappa mappa.csv] [--report report.csv] [--categoria DPI]`, **dry-run di default**): carica nel fascicolo documentale i file esportati dal vecchio gestionale, organizzati come `<Categoria>/<COGNOME_NOME>/file`. Ogni categoria va in una cartella di primo livello, riusando quella esistente con lo stesso nome (Contratti, Documenti personali, Comunicazioni sicurezza, DPI, Visite mediche; **Richiami** e **Infortuni** come cartelle riservate `solo_admin`, mai importate in una cartella omonima non riservata; i documenti di un import precedente in un'altra cartella vengono spostati). I documenti delle cartelle riservate li vedono superuser, amministratori del portale e i ruoli abilitati al permesso canonico `anagrafica.documenti_riservati.view` (scheda, download, archivio, export); i file delle visite mediche sono referti (`VISITA_MEDICA_REFERTO`, visibili solo con il permesso visite). La cartella persona si abbina al dipendente per nominativo **esatto** (stesse parole in qualunque ordine, senza accenti): i nomi non abbinati o ambigui non si importano e si risolvono con una mappa CSV `cartella;legacy_id`. Stesse regole dell'upload manuale (estensioni ammesse, firma del contenuto, 50 MB); rieseguibile senza doppioni; una voce di audit per dipendente.
- **Storico visite dai referti dell'archivio HR TOOLS** (`python manage.py leggi_referti_archivio [--apply] [--report report.csv] [--limite N] [--cartella "<estrazione>\VISITE MEDICHE"]`, **dry-run di default**): legge con l'OCR dell'acquisizione referti i certificati importati nel fascicolo. Per ogni certificato di idoneità registra **una sola visita** (la riga «Visita Medica» del protocollo, con data ed esito del giudizio) e salva **tutte le righe del protocollo sanitario come requisiti del dipendente** (`RequisitoVisitaDipendente`: vale l'ultimo certificato, i precedenti restano come storico). I requisiti entrano nello scadenzario e nella conformità accanto a ruoli operativi e processi MOD.128; le visite mancanti non generano promemoria. I **certificati oculistici** (data, nome ed esito scritti a mano) vanno nella coda referti, dove si inseriscono data, tipo di visita (proposto dal requisito oculistico) ed esito. La persona è quella della cartella di provenienza: nominativo e data di nascita letti servono a confermarla. Si registra da solo solo ciò che è certo (identità confermata, «Visita Medica» e giudizio riconosciuti dagli alias; un requisito non a catalogo non blocca); il resto va nella coda di revisione, già agganciato al documento. Il riepilogo elenca esami e giudizi da mappare negli alias prima dell'`--apply`; rieseguibile (pagine già lette saltate). `--cartella` permette la prova sulla cartella dell'estrazione prima dell'import.
- **Scheda della singola visita medica** (`/anagrafica/visite-mediche/<id>/`): da **Apri visita** nelle scadenze o dalla data nelle ultime visite e nello storico dipendente si apre un riepilogo HTML stampabile con esito, scadenza, note e referti protetti. La scheda permette di correggere la registrazione; il pulsante di eliminazione motivata compare solo con il permesso `anagrafica.visite.delete`. L'accesso segue il gate sanitario della sezione.
- **Gestione del singolo referto** (`/anagrafica/visite-mediche/referti/<doc_id>/gestione/`): dall'ingranaggio accanto a ogni referto (scheda visita, scheda dipendente, dashboard visite) si apre la scheda del documento sanitario. Permette di correggere nome, descrizione e data di conservazione GDPR, **sostituire il file** conservando lo stesso record (il precedente viene rimosso dall'archivio privato), cambiare il ruolo sulla visita (principale/secondario), **scollegarlo** con motivo (il referto resta nel fascicolo del dipendente) ed **eliminarlo** con motivazione obbligatoria. La scheda mostra **chi ha aperto il referto e quando** (audit dei download) e lo storico completo delle operazioni sul record. Lettura con il gate sanitario della sezione, eliminazione con `anagrafica.visite.delete`.
- **Visite mediche** (`TipoVisitaMedica`, `VisitaMedica`, gated `AnagraficaVisiteMedichePermission`): catalogo tipologie con `durata_mesi` e M2M verso `RuoloOperativo` (la visita è obbligatoria per il dipendente se ha almeno un ruolo collegato). Registrazione visita con esito (idoneo/idoneo con prescrizioni/non idoneo) e referto allegato opzionale (storage privato). La `data_scadenza` è calcolata in `save()` come `data_svolgimento + durata_mesi` (helper Python-only `_add_months`, nessuna dipendenza esterna). Servizio `services/visite.stato_visite(legacy_id)` produce gli stati `mancante`/`valida`/`in_scadenza`/`scaduta`. Management command `send_visite_expiry_reminders --days 60` per il digest email + notifica al dipendente. Default permesso: solo superuser + admin legacy (dato sanitario). **«Giornata visite» reattiva e multi-tipo** (`/anagrafica/visite-mediche/nuova-sessione/`, modello `VisitaSessione`): data + medico + un elenco di persone ciascuna con la propria visita dovuta (tipi misti), coi candidati caricati live via HTMX (filtro tipo senza reload, scorciatoie, barra sticky); riusa la proposta «consona» (ruoli + MOD.128, cessati esclusi, storico) e i guardrail (anti-doppione, no date future, prescrizioni/note separate, referto per riga). Le giornate sono **salvate**: **hub «Sessioni & proposte»** (`/anagrafica/visite-mediche/sessioni/`) con proposte di rinnovo per tipo e «Giornata completa», più lista e dettaglio consultabili con aggiunta partecipanti a posteriori. Punti d'ingresso anche dallo **scadenzario** (pulsante «↻ Rinnovo» per gruppo, deep-link `?tipo=`). **Scadenze "confermate"**: helper unico `services/visite.ultime_visite_correnti_ids()` — tutte le viste (dashboard, scadenzario, export scadenze/copertura, digest AU45) contano solo l'ultima visita per dipendente+tipo, così registrata la nuova visita la vecchia scadenza sparisce ovunque. **Acquisizione automatica dei referti scansionati** (`/anagrafica/visite-mediche/referti/`, stesso permesso della sezione): i certificati di idoneità sono firmati dal lavoratore e arrivano quindi come PDF di sola immagine; il portale li prende da una cartella di rete (job `intake_referti_sanitari` ogni 10 min, opt-in) o da un caricamento multiplo, li legge con **PyMuPDF + Tesseract** (`-l ita`, parametri in configurazione: 200 dpi / `--psm 6`, misurati — i default 300/psm3 corrompevano la data del giudizio) e **propone**; registrare resta una decisione umana e la conferma automatica è spenta di default. Le pagine consecutive dello stesso certificato vengono analizzate come un solo referto e il PDF completo resta l'unico allegato; se lo scanner produce file distinti con nomi espliciti `pagina 1`, `pagina 2` (anche `pag`/`page`), il lotto li ricompone prima dell'OCR. Un nuovo record nasce solo davanti a un nuovo blocco anagrafico; una scansione con certificati di persone diverse continua quindi a separarli. Il nominativo si legge dal **blocco anagrafico** e non dalla riga di firma (coperta dalla firma autografa, restituisce nomi verosimili e falsi), la data del giudizio si decide **per consenso** fra le tre occorrenze del documento, e il dipendente si riconosce con fuzzy `difflib` **confermato dalla data di nascita** (`DipendenteAnagraficaCivile`): data discordante, dipendente cessato, candidati multipli o nominativo di ripiego vanno sempre in coda. Un certificato genera **una sola visita**, la riga «Visita Medica» alla data del giudizio; tutte le righe del **protocollo sanitario** diventano invece requisiti del dipendente, con scadenze calcolate dal **catalogo** (`durata_mesi`) e non dalla periodicità stampata — la divergenza viene segnalata, non sovrascritta. Esami e giudizi si traducono con **tabelle di alias modificabili in pagina** (`AliasEsameProtocollo`, `AliasEsitoIdoneita`): un nome mai visto costa una riga, non un rilascio, e ciò che non si riconosce va in revisione invece di essere indovinato. L'alias dell'esame ha come chiave **testo + periodicità**, perché a catalogo la stessa visita esiste in versioni che si distinguono solo per cadenza (Annuale/Biennale/Quinquennale) e il certificato la dichiara nella colonna accanto: si cerca prima la riga per la cadenza esatta, poi quella generica (periodicità vuota = ripiego), infine il nome a catalogo — senza, un referto quinquennale avrebbe creato una visita annuale. **Si aggancia alla visita già registrata invece di duplicarla**: se esiste già una `VisitaMedica` dello stesso dipendente e tipo, **senza referto ancora agganciato**, entro una finestra configurabile (default 7 giorni, `RefertoIntakeConfig.giorni_tolleranza_associazione`) dalla data del giudizio, il certificato vi si allega — utile perché la data registrata a mano (es. dalla Giornata visite) e quella scritta poi dal medico spesso divergono di un giorno o due pur essendo lo stesso evento; data ed esito già registrati non vengono toccati, solo annotati con lo scarto. La coda di revisione si lavora **in blocco** (`POST /anagrafica/visite-mediche/referti/azioni/`): selezione per riga con scorciatoie «Tutti» / «Solo quelli con data confermata», conferma o scarto dei selezionati da una barra sticky, con la stessa validazione e lo stesso audit per riga della conferma singola — un referto guasto non ferma gli altri e viene riportato per nome. Se una pagina/referto è comunque rimasta separata, **«Unisci con altra visita»** propone visite esistenti dello stesso dipendente, dello stesso tipo o — se `TipoVisitaMedica.categoria` è valorizzata — di un tipo qualunque della stessa categoria (es. oculistica annuale/biennale, quando la periodicità richiesta è cambiata per cambio mansione): non crea una visita, non sostituisce il referto principale e conserva ogni allegato, visibile sia nella scheda dipendente sia nella dashboard visite; il controllo viene ripetuto lato server con la stessa famiglia e l'azione è in audit. **Privacy**: il testo grezzo dell'OCR non viene mai salvato (contiene materiale diagnostico estraneo agli obblighi del datore di lavoro), sopravvivono solo i campi riconosciuti; il PDF resta nell'archivio privato cifrato fuori webroot e ogni conferma registra chi e quando. Pagine: coda di revisione, registro acquisizioni, impostazioni (cartella, parametri OCR, soglie, alias). **Richiede Tesseract 5.x + lingua `ita` installati sul server** (unica dipendenza binaria di sistema del portale; `TESSERACT_CMD`/`TESSDATA_PREFIX` nel `.env` persistente) — senza, i referti si archiviano ma non si leggono. Installazione con `tools\install_tesseract_prod.ps1` (da eseguire sull'host come amministratore): copia portable o installer, verifica della lingua **prima** di scrivere, configurazione del `.env` con backup, ACL per l'identità dell'app-pool, riavvio e prova del motore; idempotente, `-DryRun` per l'anteprima.
...
- **Conformità alla mansione** ("è in regola?"): semaforo aggregato che unisce formazione obbligatoria + visite mediche + qualifiche + DPI in un unico esito `In regola`/`In scadenza`/`Non conforme`/`Nessun requisito`. Visibile come **pannello** (lazy-load HTMX) nella scheda dipendente (tab Riepilogo) e come **report trasversale** `/anagrafica/conformita/` su tutti i dipendenti attivi con filtri reparto/esito, KPI ed export CSV (gated `_check_hr_permission`). **Dato mancante ≠ scaduto**: un requisito mai registrato (visita/DPI/corso assente) vale `N/D` (neutro), non `Non conforme` — così i dipendenti già in forza senza dati non risultano "scaduti"; per i nuovi ingressi l'adempimento è guidato dall'onboarding. Solo i record realmente scaduti (esistenti e con scadenza passata) restano `Non conforme`. Privacy: il semaforo visite mostra solo valido/scaduto, mai esiti clinici/prescrizioni; i nomi tipologia compaiono solo con il permesso visite. Servizio batch `services/conformita.py` (numero di query costante). **Idoneità alla mansione** (lente `idoneita`): oltre al semaforo, il servizio confronta i requisiti della **mansione di rischio** del dipendente con quelli posseduti → `Idoneo` / `Idoneo con riserve` (requisito mancante = avviso) / `Non idoneo` (requisito scaduto) / `Non valutabile`, con l'elenco dei requisiti da soddisfare. **Avviso + tracciamento, nessun blocco operativo.** Visibile come riga dedicata nel pannello scheda dipendente e come colonna + filtro + voci CSV nel report `/anagrafica/conformita/`. Il profilo di rischio del dipendente può essere **arricchito manualmente da un admin**, direttamente dal pannello: **esposizioni a rischio dirette** (`EsposizioneRischio.legacy_anagrafica_id`, es. carrello elevatore + lavori in quota oltre alla mansione anagrafica) e **visite mediche aggiuntive facoltative** (`DipendenteVisitaFacoltativa`, scelta volontaria HR/lavoratore non derivata da mansione/rischio, quindi esclusa dal semaforo di conformità). Entrambe sono liste con aggiunta/rimozione HTMX inline, visibili solo a chi ha già visibilità sulle visite mediche.
- **Libretto sanitario aziendale** (`/anagrafica/dipendenti/<id>/libretto-sanitario/`, login; dettaglio sanitario gated `AnagraficaVisiteMedichePermission`): la catena **fattori di rischio → mansione → obblighi → conforme/non conforme** letta per esteso su un foglio solo e stampabile (A4). Il semaforo di Conformità dice *se* la persona è in regola, il libretto dice *su cosa*, *perché è dovuto* e *entro quando*: verdetto in grande con i conteggi, i quattro passi della catena, il riquadro «da sistemare / da completare» e una tabella per dominio (**sorveglianza sanitaria**, **DPI**, **formazione**, **abilitazioni**) con una riga per obbligo — requisito, **origine** (`Mansione «X»` / `Area aziendale` / `Esposizione diretta` / `Processo «Y»`), ultima evidenza, scadenza, stato. Nessuna seconda verità: i requisiti vengono dal resolver unico `services/mansionario.py` (nuova `requisiti_dipendente_dettaglio`, che aggiunge l'origine) sommati a quelli dei processi qualificati MOD.128, e lo stato usa le stesse regole e la stessa soglia (60 gg) del semaforo `services/conformita.py`. Come lì, **scaduto ≠ mai registrato**: il primo è una violazione in atto (non conforme), il secondo un adempimento da pianificare («da acquisire»); il verdetto pesa solo gli obblighi di mansione, quindi una qualifica scaduta che nessun requisito impone non rende non idonei. **Privacy**: tipologia di visita e giudizio di idoneità solo col permesso visite (senza, restano scadenza e stato con avviso in calce); prescrizioni del medico competente mai. Accesso dalla testata della card «Conformità alla mansione» nella scheda dipendente e dal pannello conformità. Servizio `services/libretto_sanitario.py`. **Quadro generale** `/anagrafica/libretto-sanitario/` (gate HR, voce «🩺 Libretto sanitario» nella sotto-nav Salute e Sicurezza): il resoconto aziendale. Il report Conformità conta le **persone** per semaforo, qui si contano gli **adempimenti** — l'unità di misura del lavoro da fare. KPI (scaduti · in scadenza 60 gg · da acquisire · **% di copertura**), barra di composizione per ambito («dove si concentra il lavoro»), filtri **reparto / mansione / esito persona / ambito / stato adempimento**, export CSV e stampa A4 orizzontale. Due letture della stessa lista: **una riga per persona** (verdetto, contatori per stato, prime criticità) e **una riga per requisito** (`?vista=requisiti`: scaduti e scadenze più vicine in testa — la lista di lavoro di RSPP/HR). **Export PDF/Excel** dall'endpoint unico `anagrafica:export` (spec `libretto_sanitario_persone` e `libretto_sanitario_requisiti`): il PDF è il documento tabellare NOVICROM HUB (`core/table_pdf.py` — logo, A4 orizzontale, filtri dichiarati in testa, audit dell'export), non la stampa della schermata. Pagina, PDF ed Excel chiamano la stessa `quadro_generale()`, quindi il documento non può divergere da ciò che si vede; numero di query costante (`libretto_batch()`, su cui il libretto individuale è un wrapper).
...
- **Numerazione automatica a tre livelli — corso, edizione, lezione** (migration `0123`, `services/formazione_numerazione.py`): il codice non si digita più, lo alloca il portale. Il **corso** prende `YYNNN` a cinque cifre — le prime due sono l'anno di creazione, le ultime tre il progressivo di quell'anno: `26066` è il 66° corso creato nel 2026. Il contatore è una riga per anno (`TrainingCourseCodeCounter`) allocata in transazione con `select_for_update`, così due salvataggi simultanei non si contendono lo stesso numero, e i codici già occupati (dati importati) vengono saltati. L'**edizione** aggiunge l'anno di *erogazione* e un progressivo che **riparte a ogni anno solare**: `26066-26E1`, `26066-26E2`, poi `26066-27E1` a gennaio — il codice del corso porta l'anno di nascita e non cambia più (qualifiche, requisiti e attestati storici sono agganciati lì), mentre l'anno sull'edizione risponde a «quante ne abbiamo erogate nel 2027» senza aprire le date. La **lezione** prende il primo `numero` libero dell'edizione (si guarda il massimo in uso, non il conteggio: cancellare la giornata 2 di tre non fa rinascere un numero già stampato sul registro firme). **I codici storici restano**: `SIC-1`, sigle dal titolo e codici d'importazione non vengono ricodificati — sono citati su attestati emessi, export e MOD.128 — e le edizioni vecchio formato `<CORSO>-E<N>` non spostano il progressivo del nuovo.
- **Materiale del corso, riproposto alla nuova edizione** (`TrainingAttachment` a tre livelli): dispense, programma e modulistica si caricano sulla **scheda del corso** (`/anagrafica/formazione/corsi/<id>/`) con la spunta «Proponi alle nuove edizioni». Alla creazione di un'edizione quel materiale compare **pre-spuntato**, insieme alla casella «Riporta le note del corso»: si deseleziona ciò che non serve e il resto viene **copiato**, non collegato — come già avviene per il programma didattico, perché l'edizione deve poter documentare ciò che ha davvero consegnato in aula anche quando il corso, mesi dopo, cambia dispensa (`copiato_da` tiene la discendenza, così la stessa dispensa non viene riproposta due volte sulla stessa edizione). Un solo modello copre corso, edizione e lezione: stessa view di upload, stesso archivio privato fuori webroot, stessa ACL e stesso audit del registro firme.
...
- **Creazione corso in un passaggio, ore al netto della pausa, corso senza sessione** (`services/formazione_pianificazione.py`, migration `0096`): il flusso corso → sessione → lezioni non è più obbligato in tre tappe. **(1) Pausa**: `TrainingLesson.pausa_minuti` scala il tempo non formativo dal monte ore — una giornata **08:00–17:00 con 60′ di pausa vale 8 ore, non 9** — e poiché `durata_ore` era già la fonte unica del calcolo, la correzione arriva da sola a percentuale di presenza, registro firme, attestato ed export; il dettaglio sessione confronta le ore pianificate con la **durata teorica del corso** e segnala lo scostamento. **(2) Sessione unica**: il wizard `/anagrafica/formazione/corsi/nuovo/` ha la tappa facoltativa **«Programmazione»** (data, orario, pausa, sede, docente) che crea corso + sessione + giornate in un'unica POST, con riepilogo live `N giornate × Xh = totale` che preimposta la durata teorica; il **codice edizione** è automatico (`<CORSO>-<YY>E<N>`, vedi *Numerazione automatica*) e la data di fine vuota vale «un giorno solo». Nel dettaglio sessione **«Genera giornate»** crea una lezione per giorno dell'intervallo che cade nei **giorni della settimana** scelti (o per ogni **data puntuale** indicata) con lo stesso orario-tipo, ed è rilanciabile: salta i giorni già coperti. **(3) Corso senza sessione**: per un corso frequentato presso un ente esterno o recuperato dallo storico, la scheda del corso offre **«Registra completamento senza sessione»** — il `TrainingEmployeeRecord` nasce con `sessione=NULL`, è idempotente su corso × dipendente × data e passa dagli stessi effetti (scadenzario, qualifica àncora, archiviazione attestato) del completamento d'aula.
...
- **Motore manutenzioni Piano → Applicazione → Occorrenza → OdL**: la scadenza non vive più dentro l'ordine di lavoro ma in un'**occorrenza** (`MaintenanceOccurrence`), cioè «il piano X va eseguito sull'asset Y entro la data Z». Togliere un asset da un OdL massivo non cancella più la manutenzione: l'occorrenza torna «da pianificare» e continua a comparire fra le manutenzioni dovute. Il **Piano di manutenzione** dice *cosa* fare (checklist, interna/esterna, allegato obbligatorio, tipo — inclusa la **scadenza amministrativa**, che smette di essere un sistema a parte); l'**applicazione** (`MaintenancePlanAssignment`) dice *dove* e *con che tempi*, quindi lo stesso piano vale su gruppi diversi con periodicità diverse (TORNI ogni 30 giorni, FRESE ogni 90, DMG-04 ogni 45). La precedenza è **asset > gruppo > categoria**; se due gruppi impongono periodicità diverse allo stesso asset il sistema **dichiara il conflitto** invece di sceglierne una in silenzio. I **gruppi asset** (`AssetGroup`) danno la famiglia operativa che la categoria non può dare, perché un asset ne appartiene a più d'una. Periodicità **solo di calendario** — giorni, settimane, mesi, anni, trimestre, semestre, «primo lunedì del mese», «ultimo giorno del mese», annuale a data fissa — con due ancoraggi: la manutenzione ordinaria riparte dalla **data di esecuzione**, la scadenza amministrativa resta ancorata alla **scadenza teorica** (rinnovare una polizza in ritardo non sposta in avanti tutti gli anni successivi). Comandi: `generate_maintenance_occurrences` (scheduler idempotente, `--dry-run`), `manut_audit` (diagnosi di **sola lettura**: debito storico, uso reale, conflitti, e con `--runtime` / `--sorgenti` / `--fonti-parallele` lo stato reale dello scheduler django-q, il motore che ha generato ogni occorrenza e le sovrapposizioni fra fonti di scadenza) e `migrate_maintenance_to_plans` (conversione di regole, override, ultima esecuzione e scadenze amministrative, `--dry-run` e conteggi). Le manutenzioni a **contatore** (ore/km/cicli) escono dal flusso: senza letture attendibili producevano scadenze false presentate come verdi. Le pagine ci sono: **«Da fare»** (`/assets/manutenzione/da-fare/`) è la giornata del manutentore — scadute, entro 7 giorni, programmate, in attesa, esterne, ogni blocco apribile e richiudibile dal titolo con la scelta ricordata — con raggruppamento per piano, famiglia o asset e selezione multipla, disponibile sia nei blocchi per urgenza sia nei gruppi, che apre **un solo ordine di lavoro per più macchine**; **«Scadenze»** la vista temporale con tutti i filtri; **«Cruscotto»** (già «Quadro», URL invariato) il pannello del responsabile, che mette in evidenza cosa è *dovuto ma non ancora pianificato*, i rapporti mancanti, i conflitti e il carico dei manutentori. Il Cruscotto ha **due letture sullo stesso URL** (`?vista=`): **Operativo** — le liste di cosa fare — e **Sintesi**, la lettura di andamento per chi deve rispondere del modulo: manutenzioni **nei tempi** sugli ultimi 12 mesi — calcolate **solo dove scadenza ed esecuzione sono due eventi distinti**, perché nelle occorrenze migrate dal vecchio motore la scadenza è stata dedotta dall’esecuzione e risulterebbero puntuali per costruzione; sotto una base minima di dieci righe la percentuale non si mostra affatto — scadute con l’anzianità della più vecchia, copertura documentale, quota di parco con almeno un piano, e un grafico a barre delle concluse per mese diviso fra puntuali e in ritardo. La Sintesi **non mostra costi, fermo macchina, MTTR/MTBF né disponibilità impianti**: quei campi sono facoltativi in chiusura e quasi mai compilati, quindi al loro posto c’è un riquadro **«Cosa il portale non può ancora misurare»** che ne dichiara la copertura reale — un grafico piatto a zero sembrerebbe un risultato. Il default della vista segue i permessi già esistenti (chi non può né pianificare né eseguire apre sulla sintesi) e la scelta esplicita vince sempre. Ogni fornitore ha la sua **scheda di manutenzione** (`/assets/manutenzione/fornitori/<id>/`): contratti di assistenza, manutenzioni affidate, verifiche periodiche, interventi aperti e conclusi, asset coperti — con la spesa **non stampata** ma dichiarata come copertura del dato, per lo stesso motivo; l’anagrafica del fornitore resta nel modulo `/fornitori/`. Nel dettaglio dell'ordine di lavoro il pannello «Manutenzioni raccolte» è raggruppato per giornata, conta «completate · da fare · totale iniziale», e permette di registrare il singolo asset, **registrare in blocco le selezionate** (giorno, note, fermo e un eventuale rapporto unico dichiarati una volta sola, ma chiusura e prossima scadenza calcolate per singolo asset; quelle che richiedono un documento obbligatorio vengono saltate e dette per nome), aprire un follow-up sull'anomalia, distribuire il lavoro su più giorni e **togliere un asset senza chiudere la sua manutenzione**. Chiudere l'intervento non registra le manutenzioni che raccoglie: se ne restano di aperte il portale lo dichiara in chiusura e nel pannello, perché altrimenti quelle scadenze resterebbero scadute in silenzio. La configurazione (**Piani**, **Gruppi asset**, personalizzazione per macchina, **matrice di copertura** ✓/P/X/!/–) parla di piani e periodicità: «override», «regola» e «soglia» non compaiono. Tre permessi concedibili dal pannello Accessi: configurazione, pianificazione (`maintenance_planning`), esecuzione (`maintenance_execute`). Nella **scheda di una macchina** compaiono i piani che la riguardano, con periodicità, origine («Ereditato dal gruppo», «Personalizzato», «Escluso», «Conflitto») e prossima scadenza; su uno **step di checklist fuori range o saltato** si apre il follow-up direttamente da lì, scegliendo su quale macchina quando l'ordine di lavoro ne raccoglie più d'una. La **coda degli interventi è condivisa**: un ordine di lavoro senza assegnatario riguarda tutti e compare in «Il mio turno» di chiunque, marcato «Di nessuno» — non si prende in carico, si apre e si fa. L'intestazione avviene **alla chiusura**, a chi chiude (`assigned_to` ed `executed_by` si valorizzano se erano vuoti), con una casella spuntata di default che compare solo per gli interventi di nessuno e si può togliere quando si registra il lavoro di un altro. «Il mio turno» non stampa le sezioni vuote — quello che manca si riassume in una riga — e ordina il lavoro per urgenza: Bloccati, Emergenze, Scaduti, Oggi, In corso, il resto della coda. Per il **caporeparto** «Da fare» e «Scadenze» preimpostano il filtro sui reparti che guida, dichiarandolo in pagina con «Mostra tutti i reparti» accanto: una preimpostazione, non una barriera per **guardare**. Per **pianificare** invece sì: chi guida uno o più reparti (e non configura i piani) crea ordini di lavoro, o vi aggiunge manutenzioni, solo sugli asset dei propri reparti — un asset di un altro reparto dà **403** con il motivo. Chi pianifica senza guidare alcun reparto (es. ufficio manutenzione) non ha questo vincolo. Il **promemoria quotidiano** legge le occorrenze appena il nuovo dominio è popolato e le sorgenti legacy tacciono, con i blocchi scadute · eseguite ma senza rapporto · in scadenza; lo scheduler crea le scadenze (`assets_generate_occurrences`) invece di aprire un ordine di lavoro per ogni asset. L'**import dello storico** (`/assets/manutenzione/importa-storico/`, comando `import_maintenance_history`) carica asset · piano · ultima esecuzione · note da Excel o CSV con modello scaricabile, anteprima riga per riga ed errori spiegati: senza l'ultima esecuzione ogni piano risulterebbe dovuto subito. **Guida all’uso con gli screenshot delle pagine vere**: [`docs/manutenzione/GUIDA_MANUTENZIONE.html`](docs/manutenzione/GUIDA_MANUTENZIONE.html) — tre parti, una per ciascuno dei tre permessi (manutentore · responsabile · amministratore), passo per passo, più un capitolo di domande frequenti. Avanzamento e passi rimanenti in [`docs/ai/CHECKLIST_REFACTORING_MANUTENZIONI.md`](docs/ai/CHECKLIST_REFACTORING_MANUTENZIONI.md)
...
- **Tre linguaggi visivi, una struttura sola**: le pagine del modulo non sono piu "la stessa tabella con colonne diverse". Le **operative** (`Da fare`, `Interventi`) usano la variante `md-table--op` del componente condiviso `_occurrence_rows.html` (parametro `layout="op"`): quattro colonne — Attivita / Asset / Gestione / Azione — righe piu alte, urgenza scritta in chiaro e una barra verticale di colore a sinistra (rosso scaduto, arancio oggi, blu programmato, grigio in attesa, verde concluso, da `VIEW_STATE_TONES`). Le **gestionali** (`Scadenze`, `Piani`, `Fornitori`) restano dense: senza il parametro il componente rende esattamente la tabella di prima. Le **analitiche** (`Cruscotto`, `Sintesi`) sono KPI ed eccezioni; l'**archivio** (`Storico`) e' compatto e con poco colore. Nessun dato e' stato tolto dalle viste operative: e' stato spostato di livello gerarchico.
- **Una sola coda operativa**: `Da fare` (`/assets/manutenzione/da-fare/`) apre con il blocco **"I miei interventi"** — ordini in corso, bloccati o urgenti, propri o non assegnati — che assorbe cio' che mostrava `Il mio turno`. Quella voce non compare piu nel menu (migration di soli dati `assets/0104`, reversibile), ma **l'URL `/assets/manutenzione/il-mio-turno/` continua a rispondere**.
- **"Aperti da oltre N giorni"**, non "OdL in ritardo": la soglia `assets_wo_overdue_days` (`SiteConfig`, default 21, fonte unica `get_workorder_overdue_days()`) misura da quanto un ordine e' **aperto**, non lo sforamento di una scadenza — `WorkOrder.due_at` non viene valorizzato. Cruscotto, promemoria e KPI usano ora questa dicitura.
- **"Inizia" intesta l'intervento a chi lo comincia**: la coda resta condivisa e non esiste un "Prendi in carico" obbligatorio, ma `WorkOrder.start(user)` assegna l'ordine a chi preme INIZIA **se non era di nessuno**, cosi' le liste mostrano "In corso — Nome - dalle HH:MM". Il "quando" e' `started_at`, il "chi" e' l'assegnatario: nessun campo `started_by`, nessuna migration di schema.
- **Chiusura protetta con manutenzioni ancora dovute**: chiudere un OdL **non** registra le occorrenze raccolte (ogni asset avanza sul suo piano quando quella manutenzione e' dichiarata eseguita). Se ne restano di aperte, la chiusura si ferma e le elenca: azione primaria "Torna alle manutenzioni", secondaria "Chiudi comunque" — con un click per chi ha `maintenance_planning`, con una **motivazione obbligatoria** (registrata nel log dell'intervento) per chi ha solo `maintenance_execute`.
- **Sintesi per direzione**: `/assets/manutenzione/quadro/?vista=sintesi` non mostra le scorciatoie operative e include, quando i dati esistono, **"Prossime scadenze amministrative"** (max 5 righe: data, scadenza, asset/fornitore) ricavate dalle occorrenze aperte di piani amministrativi — nessun campo nuovo, una query. Gli adempimenti gia' scaduti sono contati a parte e rimandano alla lista filtrata.
...
- **Categorie asset gerarchiche** e **campi dinamici** configurabili dalla tab `Categorie asset` di `/assets/impostazioni/` (le **cartelle documento** della categoria hanno una tab dedicata, `Cartelle documento`); la tab mostra impatto operativo, contatori, preview degli asset collegati in popup e azione rapida per pubblicare la categoria nel menu laterale come lista filtrata. Il catalogo CSV/XLSX puo creare categorie padre (`famiglia`) e sottocategorie (`sottocategoria`) con `import_assets_catalog` (per i file XLSX vengono elaborati tutti i fogli; `produttore`/`modello` finiscono nei campi dedicati, le altre colonne non standard in `extra_columns`)
- **Accesso a `/assets/impostazioni/`**: aperta a superuser e ruoli legacy admin, **oppure** a chi ha ricevuto il permesso dal pannello Accessi — pagina ACL v2 `assets_gestione` o azione modulo `assets`/`admin_assets`, la stessa che accende la voce di menu. Fino alla 1.5.0 il gate leggeva solo il ruolo admin, quindi il permesso concesso non produceva alcun effetto e la pagina rispondeva 403.
...
- **Manutenzione periodica su tutti gli asset non IT** (`/assets/manutenzione/verifiche/`): la pagina ha due scope, `it` (i tipi di `IT_DEVICE_TYPES`) e `production`, che è il **complemento** del primo (`NON_IT_ASSET_TYPES`). Ogni asset non IT può quindi avere un piano ricorrente — CNC, macchine utensili, carroponti, ma anche impianti generici e prodotti chimici — e i tipi aggiunti in futuro a `Asset.TYPE_CHOICES` vi rientrano automaticamente. Il filtro `?asset=<id>` redirige allo scope di appartenenza dell'asset **solo se diverso** da quello corrente: prima un asset fuori da entrambe le whitelist rimbalzava sulla stessa URL all'infinito.
- **Scadenzario unificato** su `/assets/manutenzione/prossime/`: elenca le regole manutenzione **a giorni**, dalla data dell'ultima esecuzione. Le soglie **a contatore** (ore/km/cicli) sono uscite dal flusso manutentivo: senza letture attendibili producevano scadenze false presentate come verdi, e non compaiono più nello scadenzario né generano interventi. Elenca anche le **verifiche periodiche pianificate** (sezione dedicata) con stato `Scaduta / In scadenza / Pianificata`, filtri condivisi (asset, status, ricerca) e link diretti al piano. I KPI di sintesi sommano regole + verifiche periodiche
...
- **Righe non valutabili in cima**: le manutenzioni mai eseguite (stato `missing`) sono in testa allo scadenzario con badge rosso — non sapere *se* una manutenzione è scaduta è più grave che saperla scaduta
- **Reminder manutenzione** (`send_maintenance_reminders`, cron 07:00): le scadenze **superate** restano nella mail — in una sezione **SCADUTE** in cima — finché non sono risolte (prima sparivano il giorno dopo la scadenza); include le manutenzioni non valutabili; esclude le verifiche `is_legacy` già coperte dalle regole (niente doppio conteggio); **notifica il manutentore assegnato** (`WorkOrder.assigned_to`) alla creazione dell'OdL, quando qualcun altro glielo porta via e quando il suo intervento è in ritardo. **Cadenza anti-rumore**: scadute tutti i giorni, in scadenza il primo giorno di preavviso, poi settimanale, poi il giorno stesso (`--no-throttle` per mandare tutto). Soglie da SiteConfig: `assets_reminder_days` (30), `assets_wo_overdue_days` (21)
...
- **Kickoff = progetto** con numerazione automatica `KICK-OFF <progressivo>` e nome proposto **`Cliente · P/N · KICK-OFF <progressivo>`** (campo *Nome del kickoff* nel form di creazione: vuoto usa la proposta, che si vede come segnaposto mentre si compilano cliente e P/N). I kickoff già esistenti non vengono rinominati dal salvataggio: per riallineare lo storico c'è `python django_app\manage.py rinomina_kickoff` — **dry-run per default**, scrive con `--apply`, e tocca solo i nomi ancora automatici (`KICK-OFF n` o vuoti), lasciando stare quelli scritti a mano (`--all` per forzarli)
- **Team di progetto a quattro ruoli, ognuno con piu' persone**: `project_managers`, `capi_commessa`, `programmers` e `caporeparti` sono elenchi (M2M), non piu' referenti singoli — una commessa puo' avere due capicommessa o due programmatori che si alternano. Il form «Nuovo kickoff» rende ogni ruolo come lista di checkbox filtrabile; serve almeno un referente in uno qualsiasi dei quattro ruoli. Le regole di accesso per ruolo (`TaskRoleAccessRule`) valgono per ogni persona dell'elenco. La readiness continua a chiedere PM + capocommessa + programmatore
- **Prontezza all'avvio (readiness)**: gate a 4 criteri calcolato al volo (VRF a posto · incontro di kickoff **svolto**, non solo pianificato · team assegnato PM/CC/programmatore (almeno una persona per ruolo) · piano attività con scadenza), con semaforo + conteggio, checklist azionabile nell'header commessa e riepilogo aggregato in dashboard. Nessun campo persistente né migrazione; annotazioni `Exists` per evitare N+1; badge dark-safe sui token
- **Segnalazioni incontri in dashboard** (`/tasks/`): banda in testa con **Prossimi incontri** (data, ora, luogo, commessa e quanto manca — «Oggi», «Domani», «Tra N giorni») e **Incontri da gestire** (esito mai registrato per un incontro già passato, minuta ancora da approvare, problemi rimasti aperti), con l'azione diretta accanto alla riga. Lo scope è quello ACL delle commesse; il taglio è personale (convocato, presente, creatore o membro del team) e diventa l'intero perimetro per chi ha `tasks_admin`. Le azioni di scrittura compaiono solo a chi può gestire quella commessa. Logica isolata in `tasks/meeting_alerts.py`
- **Data di inizio del kickoff**: è il **primo incontro non annullato**, mostrata sulla card di portfolio (con «da pianificare» quando manca), sulla card di board, fra le metriche della Panoramica e fra le pill dell'elenco incontri
...
- **Suggerimenti dell'ordine del giorno**: nella convocazione il campo *Titolo* del nuovo punto propone i punti già usati negli **altri incontri, di qualunque commessa**, ordinati per frequenza d'uso e con la durata in etichetta; scegliendone uno si precompilano nota e durata dell'uso più recente, senza mai sovrascrivere quello che si sta scrivendo. Esclusi i punti generati in automatico da problemi e azioni aperte
- **Conduci incontro** (`/tasks/projects/<id>/incontri/<mid>/conduci/`, pulsante sul dettaglio di un incontro non ancora svolto): i punti in sequenza con barra tempi (pianificato vs trascorso, avviso di sforamento), nota per punto in autosave, spunta «trattato», tempo effettivo, **attività collegata** (menu delle attività della commessa, oppure **+ Nuova attività** che la crea con titolo precompilato dal punto, responsabile, scadenza e priorità e la collega senza lasciare la pagina) e cattura rapida di azioni/decisioni/problemi. Alla chiusura la pagina esito parte con il verbale già compilato dalle note dei punti
- **Punto ODG aperto in riunione** (widget «Aggiungi un punto», presente sia in `/conduci/` sia nella pagina esito): un argomento che nasce mentre si parla entra subito nell'ordine del giorno invece di finire nelle note libere, quindi conta per spunte, tempi effettivi, minuta e riporto al prossimo incontro. È scritto lato server al momento, marcato come nato in riunione, e la riga compare senza ricaricare la pagina — in conduzione già con nota in autosave, spunta e tempo effettivo
- **Attività kickoff dall'incontro, con attrezzatura** (pulsante «Nuova attività / aggiorna esistente» nel blocco Azioni della pagina esito, e sul dettaglio incontro): un solo pannello per titolo, assegnatario, **tipo attività**, **data inizio**, data fine, priorità e **attrezzatura** — nessuna azione, collega esistente, richiedi nuovo attrezzo per P/N, richiedi verifica disponibilità: le stesse quattro modalità del form attività completo. Scegliendo un'**attività già creata** il pannello la aggiorna invece di crearne un'altra, che è il modo per correggerne la data inizio senza uscire dall'incontro
- **Data inizio correggibile dalla scheda attività** (`/tasks/<id>/`): «Data inizio» ha il suo form in linea accanto a quello della data fine, con lo stesso permesso; la data fine deve restare successiva a quella di inizio
...
- **Cancellazione da Impostazioni → Record**: ogni riga (kickoff e attività) ha «Elimina» con **motivazione obbligatoria** scritta nella riga stessa; senza motivazione la richiesta è respinta. L'audit registra chi, cosa, il perché e quante attività/incontri sono stati portati via dal cascade. Rotte `POST`-only, stesso permesso della pagina impostazioni
...
- **Incontri di avanzamento in due tempi**: ogni kickoff ha incontri numerati con uno **stato** esplicito (Pianificato / Svolto / Annullato). La **convocazione** (`/tasks/projects/<id>/incontri/<id>/edit/`) raccoglie dettagli, partecipanti portale/esterni, sale riunioni configurabili, agenda strutturata e sync Outlook; l'**esito** (`/tasks/projects/<id>/incontri/<id>/esito/`) raccoglie verbale, spunta dei punti trattati, chiusura dei problemi e next steps, e porta l'incontro in «Svolto». I problemi non risolti vengono riportati automaticamente nell'ordine del giorno dell'incontro successivo e possono essere chiusi/riaperti dall'esito o dal dettaglio. Dalla pagina dell'incontro un pulsante **"Crea / assegna attività"** apre il modal di creazione task (riusa il modal dei next steps) **anche quando l'incontro non ha next steps**. Il **sync Outlook** della convocazione richiede che l'app Graph abbia `Calendars.ReadWrite` come permesso **Application** (con consenso amministratore) e che l'**organizzatore** sia una mailbox personale con licenza: una casella di funzione o condivisa (`it@`, `info@`…) viene rifiutata da Graph — si indica quella giusta nel campo «Email organizzatore». Il sync non è bloccante: se fallisce, l'incontro resta salvato, con un avviso che dice quale delle due cause è — la convocazione via email è un canale distinto (SMTP) e si invia a parte, col pulsante **Invia convocazione** del dettaglio incontro. La mail di convocazione porta **cliente e numero KICK-OFF in oggetto**, e in tabella **data e ora su righe distinte** (`—` se l'ora manca). I **destinatari in copia** si scelgono nella convocazione (blocco «Destinatari in copia — CC»: utenti portale con ricerca, più indirizzi liberi) e valgono anche per la minuta. L'ordine di precedenza è **CC dell'incontro → CC predefiniti** (*Impostazioni KICK-OFF → Configurazione → Destinatari in copia predefiniti*) **→ PM e capo commessa**. Negli elenchi di checkbox gli utenti compaiono come «Nome Cognome - email», non con lo username.
...
- **Sidebar a sezioni**: la navigazione e' divisa in **Navigazione** (`Cruscotto`, `Inventario completo`), **Strumenti e gestione** (`Manutenzione` e gli altri strumenti) e **Inventario per categoria**, piu `Impostazioni` in Amministrazione per chi ha il permesso. Le **categorie asset** hanno una sezione propria in fondo al menu: sono un filtro sull'inventario, non una destinazione, e tredici radici come voci di primo livello dentro `Navigazione` spingevano sotto la piega le pagine che si usano ogni giorno. Non sono annidate sotto un unico genitore perche' il guscio rende due soli livelli: le sotto-categorie sparirebbero. Il ramo **Manutenzione** segue l'ordine della barra di sezione: Da fare · Scadenze · Quadro · Interventi · Piani · Gruppi asset · Storico · Manutenzione periodica produzione · Report asset produzione · Fornitori · Impostazioni (le voci di sola configurazione — regole e catalogo attivita' — vivono dentro Impostazioni e restano riattivabili da *Impostazioni > Menu sidebar*). In cima al menu c'e' un **filtro voci**: nasconde senza riordinare, apre i rami con figli corrispondenti, Esc azzera. Voci legacy/orfane basate su `asset_type` (es. "Asset Infrastruttura"/"Carroponti e paranchi") rimosse perche' duplicavano le categorie d'inventario
...
- **Calendario** con vista mensile/settimanale e colori per tipo, **a tutta larghezza** della finestra
- **Chi vede le assenze altrui — tre livelli** (`view_scope`): **tutta l'azienda**, **solo il proprio reparto** (i dipendenti di cui si e' capo assegnato: lo stesso insieme che si puo' approvare) oppure **solo le proprie richieste**. Il livello viene da due capability assegnabili in `/admin-portale/accessi/` **a un ruolo o alla singola persona** (override): `view_all_assenze` e `view_reparto_assenze`. Vale su calendario, dashboard segnalazioni ed export CSV, che applicano il filtro **in SQL** (fail-closed: senza criteri di assegnazione non si vede nulla). **Vedere non e' approvare**: Approva/Rifiuta restano legati ai permessi di moderazione, cosi' DIREZIONE/HR possono avere la vista aziendale in sola lettura. Finche' le capability non sono assegnate vale il fallback storico (amministrazione → tutto, caporeparto → reparto, altri → solo le proprie); un dipendente senza ruoli non vede il calendario. Il riepilogo di chi ricade in quale livello e' in **Impostazioni assenze → «Chi vede le assenze altrui»**, in sola lettura
- **«Le mie richieste»** (`/assenze/le-mie-richieste/`) distingue le assenze intestate a sé dalle **«Richieste dei miei dipendenti»**, dove la coda da decidere è seguita da un blocco richiudibile «Già decise» (prima una richiesta approvata spariva dalla pagina)
- **Dashboard CAR** (`/assenze/car/dashboard`) con la coda in cima e le diagnostiche in fondo; sulla coda, ricerca testuale e filtri per tipo e periodo (inizia oggi · entro 7 giorni · già iniziate), applicati senza ricaricare. Approva/Rifiuta direttamente dalla riga
- **Certificazione presenza** come tipo applicativo dedicato (persistita come `Altro` con metadato interno)
- **Sync bidirezionale in background** con lista SharePoint via Graph API, per far convivere il portale con le vecchie app SharePoint: **nessuna pagina chiama Graph**. Le modifiche fatte sul portale (richiesta, approvazione, modifica, eliminazione) entrano nella coda `AssenzaSharePointOutbox` e partono subito in background; il job django-q `assenze_sharepoint_sync` (ogni 5 minuti) ritenta la coda e poi legge da SharePoint **solo gli elementi cambiati** (delta query), eliminazioni comprese. Un record con modifiche ancora in coda non viene sovrascritto dalla lettura. Gli elementi SharePoint non collegati che il portale ha già (stesso nominativo, stesso giorno di inizio e di fine) vengono **scartati**, non uniti né duplicati. **Ogni richiesta la gestisce il sistema in cui è nata**: quelle create su SharePoint le approva il flusso Power Automate e sul portale sono in sola lettura (etichetta «Gestita su SharePoint», endpoint 409); quelle create sul portale le gestisce il portale e il flusso deve ignorarle (condizione sul trigger: autore diverso da «App di SharePoint»). Le scritture della sincronizzazione non fanno partire le automazioni del portale (flag di sessione SQL `hub_skip_automation` letto dai trigger `trg_assenze_automation_*`), così le mail non partono due volte. All'invio il portale compila i lookup SharePoint cercandoli per email: `Capo Reparto` nella lista Caporeparto (`IndirizozEmail`, capo = responsabile dell'area aziendale) e `Nome` nella lista DIPENDENTI (`USERNAME` o nominativo); le corrispondenze mancanti compaiono nel Run-log come `senza_capo` / `senza_nome` Ritardo massimo tra i due mondi: ~5 minuti. Coda ed eventuali errori sono visibili nelle Impostazioni assenze; il job si spegne dalla Centrale di comando. `ASSENZE_SYNC_ON_PAGE_LOAD` non è più letto
- **Capo reparto** nella richiesta letto da Anagrafica HR seguendo la catena canonica `dipendente → area aziendale → reparto`: l'approvatore predefinito e' il **responsabile dell'area aziendale** del dipendente (`AreaAziendale.responsabile_legacy_id`), il caporeparto del reparto e' il fallback quando l'area non ne ha uno proprio — stessa regola mostrata in Anagrafica/organigramma. Solo se la catena canonica non risolve si usa il campo denormalizzato `DipendenteAnagraficaAziendale.caporeparto_legacy_id`, poi il testo legacy del reparto, infine il fallback compatibile verso `capi_reparto`. Il **menu** dei capi elenca sia i caporeparto dei reparti sia i **responsabili delle aree aziendali** (prima solo i primi: un responsabile d'area non aveva una voce selezionabile e la richiesta ricadeva sul caporeparto del reparto). La stessa regola vale al contrario per l'elenco dei dipendenti che un capo puo' gestire e per l'escalation al superiore quando il capo e' assente
- **Inserimento "per conto di"**: Caporeparto e Amministrazione possono creare richieste **per tutti** i dipendenti, sé compresi (prima il Caporeparto era ristretto al proprio reparto); gli altri profili solo per se stessi. Lo scope è applicato sia nel form sia in fase di submit/API. Poter inserire non è poter approvare: l'approvatore resta quello risolto dall'area aziendale del dipendente scelto
...
- **Ora di fine riallineata, non rifiutata**: se l'ora di fine non è successiva a quella di inizio, viene portata a **inizio + 30 minuti sul giorno dell'inizio** (`AUTO_FINE_MINUTI`), con avviso esplicito a chi compila — sia lato server (invio e modifiche) sia nel form. Le richieste su più giorni non vengono toccate
- **Flessibilità riservata e a orari fissi** (`FlessibilitaAbilitato`, `FlessibilitaImpostazioni`): può richiederla solo chi è nell'elenco degli abilitati, negli orari di entrata/uscita decisi dall'Amministrazione (default 07:00/08:00/09:00 e 16:00/17:00/18:00). Entrambe le cose si configurano in **Impostazioni assenze → Flessibilità**, senza rilascio. Verifica **fail-closed**: senza un id anagrafica risolvibile la richiesta viene rifiutata. Restano i limiti di durata (almeno 9 ore, massimo 10) e le 2 flessibilità a settimana, quindi non tutte le combinazioni di orari ammessi passano
...
- **Pannello «Gestione admin · Tutte le assenze»** (in Impostazioni assenze): si apre sulle sole richieste **In attesa** («Tutti gli stati» per lo storico completo), con **moderazione in blocco** (selezione multipla, «Approva/Rifiuta selezionate» con nota obbligatoria, massimo 200 per volta, stessi controlli dell'azione singola su ogni record), **ordinamento cliccabile** su tutte le colonne principali e colonna **«Inserita il»**. Filtri e **paginazione lato server** (25/50/100/200 righe, default 25) invece del caricamento in pagina dell'intero storico. Ricerca su dipendente, tipo e motivo; filtri per stato, tipo e periodo dal/al (i periodi si intersecano: una richiesta iniziata prima della finestra ma ancora in corso resta visibile). Le card KPI sono link che filtrano per stato; piede con «da–a di N» e navigazione fra pagine
...
- **Impostazioni** su `/gestione-anomalie/configurazione`:
  - tab **Configurazione** — le liste **Capi reparto (CAR)** e **Capi commessa** si compongono scegliendo **quali Ruoli Operativi** del catalogo Anagrafica le formano (es. CAR = *Caporeparto*, Capicommessa = *Project Engineer*), con **override per singola persona** (nomi aggiunti a mano e nomi esclusi, scelti dai dipendenti attivi). Senza alcun ruolo selezionato resta la derivazione storica (caporeparto dei Reparti / reparto capocommessa). La composizione sta nel JSON delle liste (`derived_sources`); l'elenco risultante è sempre calcolato dal server
  - tab **Permessi** — sotto-tab `Ruoli operativi` (sola lettura: catalogo + assegnazioni dall'Anagrafica) e `Accessi`. Tutte le tabelle sono **comprimibili e chiuse di default**; i ruoli di sistema **Capocommessa** e **CAR/Incaricato** mostrano i Ruoli Operativi che li compongono, con link alla sezione di Configurazione che li definisce. Un selettore in cima sceglie **quali Ruoli Operativi compaiono nella pagina** (`permessi_ruoli_visibili`; vuoto = tutti, e un ruolo con una regola già configurata resta sempre visibile)
  - tab **Log attività** — oltre all'audit, il **registro delle email inviate dal modulo** (`AnomalieEmailLog`): tipo, destinatari, oggetto, corpo a richiesta, esito ed errore SMTP, con filtro per oggetto/OP ed esito e pulsante **Reinvia**, che rispedisce esattamente il messaggio salvato e traccia il reinvio come riga collegata all'originale
...
Archivio schede dati di sicurezza (SDS) dei prodotti chimici, assegnate alle mansioni di rischio. La mansione corrente del dipendente determina dinamicamente le SDS dovute: un cambio mansione rende disponibili le nuove schede e genera una notifica, senza cancellare lo storico delle versioni già lette.

- **3 modelli**: ProdottoChimico (M2M `anagrafica.Mansione`, M2M `dpi.CategoriaDPI` per i DPI obbligatori, `uuid` pubblico; la vecchia FK reparto resta nullable e nascosta come solo dato storico), SchedaSicurezza (versionata, una sola scheda `is_corrente` per prodotto), PresaVisioneScheda (pattern locale specchiato da `procedure_refresh`, non lo estende: la presa visione SDS è legata alla singola versione, non a una campagna)
...
- **Presa visione HTMX** idempotente per (scheda, operatore): la nuova versione della scheda richiede una nuova conferma (storicizzazione automatica). `/schede-sicurezza/da-leggere/` mostra a ciascun utente le sole SDS correnti collegate alla sua mansione viva in anagrafica; nuova versione, nuova associazione prodotto↔mansione e cambio mansione notificano gli interessati
- **Report di conformità per mansione**: matrice dipendenti/conferme e CSV ragionano sulle mansioni, non sui reparti; una sezione separata segnala i prodotti attivi senza mansioni. La migrazione non indovina corrispondenze reparto→mansione: lo storico resta conservato finché il responsabile sicurezza completa la mappatura
...
- **Report compliance** (`/schede-sicurezza/report/`): prodotti attivi senza scheda corrente, prodotti senza mansioni e matrice per mansione della percentuale di dipendenti in forza che hanno confermato la presa visione (ponte account tramite `anagrafica_dipendenti.utente_id` e `Profile.legacy_user_id`), con export CSV
...
**NOVICROM HUB** · Costruzioni Novicrom SRL · `v1.4.0`
