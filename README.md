...
- **Valutazione qualità retrieval**: `python django_app\manage.py ai_eval --rag` misura il **recall@k** del RAG su un set di golden `domanda → fonte attesa` (opzioni `--top-k`, `--sources` per misurare *come in produzione* con `README.md,django_app/ai_assistant/knowledge`, `--json`); `ai_eval` senza `--rag` valuta invece il routing semantico dei tool. Entrambe le modalità sono offline-friendly (BM25) e con output ASCII-safe per console Windows
