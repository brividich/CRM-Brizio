...
- **Organigramma a diagramma** (`/anagrafica/organigramma/diagramma/`, login, toggle **🗺 Vista diagramma**): la stessa gerarchia dei ruoli disegnata come **albero verticale**, una riga per **posizione** (ruolo + persona) con avatar, nome, numero di riporti diretti e collasso del ramo. I riporti scendono indentati sotto il genitore; quando sono **più di cinque e tutti foglia** si dispongono automaticamente su **più colonne**. I ruoli con più titolari si espandono in righe sorelle; un ruolo con più titolari **e** riporti resta una riga unica (la gerarchia è tra ruoli, mai tra persone); i ruoli senza titolari restano in pianta come *posizione scoperta*. Toolbar «Espandi/Comprimi tutto». Rendering SSR senza librerie, sui token del tema (chiaro e scuro); gli avatar passano dalla view protetta `anagrafica:foto_dipendente`. Selettore **Organigramma** in toolbar per disegnare un solo **ambito** per volta (`?ambito=`). Builder `build_posizioni_albero()` + `griglia_riporti()` in `anagrafica/services/organigramma_albero.py`.
