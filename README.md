...
- **Timeline di vita con inserimento manuale**: il gruppo «Timeline ciclo di vita» della scheda asset unisce agli eventi automatici (registrazione, assegnazione, messa in servizio) le voci inserite a mano con il pulsante **«+ Inserisci»** (`AssetTimelineEntry`: data evento, titolo, etichetta, descrizione, riferimento, colore), pensate per i fatti che non passano dal portale — fermi macchina, traslochi di reparto, collaudi del costruttore, modifiche strutturali. Ogni inserimento e tracciato in audit con l'autore. **Visibilita opzionabile**: di serie amministratori/gestori asset, estendibile ai manutentori concedendo dal pannello Accessi l'azione `assets/asset_timeline_entry`; il flag di categoria **«Inserimento manuale in timeline»** (Impostazioni Assets → Categorie, attivo di default) consente di spegnere la funzione dove non serve
