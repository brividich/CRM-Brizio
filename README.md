...
- **Storico visite dai referti dell'archivio HR TOOLS** (`python manage.py leggi_referti_archivio [--apply] [--report report.csv] [--limite N] [--cartella "<estrazione>\VISITE MEDICHE"]`, **dry-run di default**): legge con l'OCR dell'acquisizione referti i certificati di idoneità importati nel fascicolo e crea le visite del protocollo sanitario con le scadenze a catalogo. La persona è quella della cartella di provenienza: nominativo e data di nascita letti servono a confermarla. Si registra da solo solo ciò che è certo (identità confermata, esami e giudizio riconosciuti dagli alias); il resto va nella coda di revisione dei referti, già agganciato al documento. Il riepilogo elenca esami e giudizi da mappare negli alias prima dell'`--apply`; rieseguibile (pagine già lette saltate). `--cartella` permette la prova sulla cartella dell'estrazione prima dell'import.
