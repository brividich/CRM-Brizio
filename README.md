...
- **Sync bidirezionale in background** con lista SharePoint via Graph API, per far convivere il portale con le vecchie app SharePoint: **nessuna pagina chiama Graph**. Le modifiche fatte sul portale (richiesta, approvazione, modifica, eliminazione) entrano nella coda `AssenzaSharePointOutbox` e partono subito in background; il job django-q `assenze_sharepoint_sync` (ogni 5 minuti) ritenta la coda e poi legge da SharePoint **solo gli elementi cambiati** (delta query), eliminazioni comprese. Un record con modifiche ancora in coda non viene sovrascritto dalla lettura. Ritardo massimo tra i due mondi: ~5 minuti. Coda ed eventuali errori sono visibili nelle Impostazioni assenze; il job si spegne dalla Centrale di comando. `ASSENZE_SYNC_ON_PAGE_LOAD` non è più letto
