...
- **Cruscotto operativo «Cose da fare» + «Segnalazioni arrivate»**: pannello a due colonne condiviso (un solo partial) in testa sia al **cruscotto Assets** (`/assets/`) sia al tab **Da fare** del Centro Manutenzione. **Cose da fare**: OdL aperti (con evidenza degli OdL in ritardo — soglia unica `assets_wo_overdue_days`, default 21 gg), scadenze amministrative e verifiche periodiche scadute, scadenze entro 30 gg, lista breve degli OdL più vecchi. **Segnalazioni arrivate**: i ticket di manutenzione (MAN) inviati dagli operatori (es. da «Segnala un problema») — nuove da prendere in carico, in carico, in attesa, urgenti (priorità alta/urgente o impatto sicurezza), arrivate negli ultimi 7 giorni, distribuzione per priorità e ultime segnalazioni; ogni numero è cliccabile verso la lista ticket filtrata. Servizi `get_cose_da_fare_overview()` / `get_segnalazioni_overview()` in `assets/services/dashboard_kpi.py`.
...
- **QR code** (`services/qr.py`) verso la vista mobile sintetica, basato su `ProdottoChimico.uuid` (mai la PK, anti-enumerazione); vista gated ACL v2 (default sicuro, dietro login) con pittogrammi, frasi H/P, DPI obbligatori con immagine, primo soccorso e incompatibilità in evidenza
- **Download PDF autenticato** da storage privato cifrato (`PrivateSchedaSicurezzaStorage`, stesso pattern di `gestione_specifiche`), mai link diretto a file su disco
