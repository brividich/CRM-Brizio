...
| 10 | [`tasks`](django_app/tasks/) | Operations | `/tasks/` | Portfolio **KICK-OFF** progetti, attività, Gantt con drag spostamento/resize, timeline eventi leggibile, incontri avanzamento con **invio email della minuta ai partecipanti** (via automazione, vedi AU52), VRF (MOD.073), **indicatore di prontezza all'avvio** (gate 4 criteri), **Centro «Da gestire»** (`/tasks/da-gestire/`), blocco progressivo, flag impatto sicurezza |
...
| 22 | [`automazioni`](django_app/automazioni/) | Automation | `/automazioni/` | Designer visuale, trigger SQL, queue processor, approvazioni email/Teams, import Power Automate, **regola «Minuta incontro KICK-OFF» (AU52)** — invio email del verbale ai partecipanti (source `tasks_kickoff` + azione custom `send_meeting_minute`) |
